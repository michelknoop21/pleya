...
- **`ubiquityIdentityToken` beantwoordt de iCloud-Drive-vraag, niet de key-value-vraag.** Het token is de identiteit van iCloud Drive-documenten, en iCloud Drive bestaat niet op tvOS: op een Apple TV die keurig is ingelogd is het altijd nil. Wie er de key-value-opslag op gate't, laat élke Apple TV "log in met iCloud" melden en zet de schakelaar uit. `NSUbiquitousKeyValueStore` heeft een eigen poortwachter, het recht `com.apple.developer.ubiquity-kvstore-identifier`, dat alle drie de Apple-targets dragen. Tweede helft van dezelfde val: een mislukte kanaalaanroep is een transportstoring, geen ontbrekend account. Houd die twee gescheiden (`ICloudSyncStatus`), anders krijgt de gebruiker inlogadvies voor een probleem dat niets met inloggen te maken heeft. Zie [DEC-037](docs/DECISIONS.md#dec-037); `test/architecture/icloud_kvs_availability_contract_test.dart` bewaakt de native kant, die geen testtarget heeft.

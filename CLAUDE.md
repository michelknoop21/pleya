...
## Pleya Server (goedgekeurde baseline, PS-0 vrijgegeven)
...
**Vrijgave is per fase.** PS-0 (Docker Foundation, `pleya_server/`) is vrijgegeven en uitgevoerd; de
afwijking staat vastgelegd in `docs/pleya-server-ps0-proposal.md`. **PS-1 is nog niet vrijgegeven en
begint pas na 2.8.0.** Werk in `pleya_server/` dat verder gaat dan de PS-0-scope is dus per definitie
te vroeg.
...
Twee gates blokkeren werk: het conflictmodel voor kijkstatus moet beslist zijn vóór PS-4, en de regel
voor de content fingerprint vóór scannerlogica die relocatie gebruikt. Zie hoofdstuk 24.2.

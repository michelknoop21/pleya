...
class NowWatchingProvider extends ChangeNotifier with DisposableChangeNotifierMixin {
  ...
  /// Repainting a panel every five seconds because a progress bar moved is the
  /// point; repainting the whole app bar for it is not. Compared on what the UI
  /// actually draws.
  static bool _sameShape(NowWatching a, NowWatching b) {
    ...
    for (var i = 0; i < a.sessions.length; i++) {
      final x = a.sessions[i];
      final y = b.sessions[i];
      if (x.id != y.id ||

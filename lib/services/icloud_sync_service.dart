...
class ICloudSyncService {
  ...
  static Future<void> start({
    ...
    // The hook goes in before the availability check, so a write in that window
    // is still stamped: an unstamped value loses to every stamped remote record.
    // Only the send waits. Until the first answer the status would read
    // `disabled`, which `starting()` promotes to `ready`, and a mutation would
    // report a send from a signed-out device; reading `unavailable` makes
    // apply() stamp and hold back instead. refreshAvailability() replaces it.
    coordinator.status.value = coordinator.status.value.copyWith(availability: PreferenceSyncAvailability.unavailable);

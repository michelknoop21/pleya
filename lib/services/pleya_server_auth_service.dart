...
class PleyaServerAuthService {
  ...
  MediaServerException _authFailure(MediaServerResponse response) {
    ...
    if (response.statusCode == 401 || response.statusCode == 403) {
      return MediaServerAuthException(
        error?.message.isNotEmpty == true ? 'Sign-in was rejected' : 'Invalid username or password',
...
/// the stored token thrown away and a fresh sign-in, and leaving the dead token
/// on disk means every launch re-triggers the revocation.

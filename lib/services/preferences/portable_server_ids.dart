...
/// | pleyaServer | `serverId` from `GET /pleya/v1/info` (`server.id`) | yes |
    ...
    for (final connection in connections) {
      switch (connection) {
        ...
        case PleyaServerConnection(:final serverId):
          // The server hands out its own identity on `GET /info`, and it
          // survives a restart and a change of address. Two devices pointing
          // at the same Pleya Server therefore agree on this id, which is
          // exactly what portability asks for.
          if (serverId.isNotEmpty) ids.add(serverId);

/// What one row of a source picker actually says, reduced to the values that
/// are present.
///
/// Pure, and separate from any picker widget on purpose: "ontbrekende metadata
/// wordt weggelaten, geen rijen vol 'Onbekend'" is a decision per field, made
/// from a [MediaItem] whose backends disagree about which fields they
/// populate at all, so it belongs somewhere it can be checked field by field
/// without a widget tree. Both the tvOS source picker and the mobile one
/// (fase 1 van het iOS Unified 2026-plan, `docs/ios-unified-2026-fase1-plan.md`
/// stap 2 en stap 7) share this derivation.
///
/// Deliberately not built from a version's display label: that kind of helper
/// falls back to an "unknown" string when it has nothing, which is precisely
/// what this descriptor omits instead.
library;
...
class SourceRowDescriptor {
  ...
  /// The remembered choice for this title ("Laatst gebruikt").
  ...
  /// usable and unremarkable. An auth error must read differently from an
  /// unreachable server, so these never collapse into one "unavailable".
  ///
...
///
/// [showBackend] is decided here rather than per row: on a group whose
/// sources are all Plex, the word "Plex" on every row is noise that pushes
/// the library name — the part that actually differs — further from the eye.
List<SourceRowDescriptor> describeSources(
...
/// Dolby Vision beats HDR10 when a file carries both, because that is the
/// mode the user gets. A file with neither says nothing at all rather than
/// "SDR": SDR is the default, and labelling the default is exactly the noise
/// this descriptor omits everywhere else.
...
/// layout. "E-AC3 Atmos" and "DTS-HD 7.1" are worth a glance; "AAC Stereo" on
/// every row of a group is not, but it is also the honest answer when that is
/// the only difference between two sources, so it stays.

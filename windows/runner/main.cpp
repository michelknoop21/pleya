...
wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev, _In_ wchar_t* command_line, _In_ int show_command) {
  ...
  HANDLE mutex = CreateMutex(nullptr, TRUE, L"com.edde746.PlexFlixNetwork.SingleInstance");
  if (GetLastError() == ERROR_ALREADY_EXISTS) {
    HWND existing = FindWindow(L"FLUTTER_RUNNER_WIN32_WINDOW", L"PlexFlixNetwork");
    if (existing) {
  ...
  if (!window.Create(L"PlexFlixNetwork", origin, size)) {
    return EXIT_FAILURE;
  }

...
void main() {
  ...
  group('cache', () {
    ...
    test('A16: a hit naming a server that left the profile is revalidated, not trusted', () async {
      await resolverFor([
        _server('s1', matches: [_item('s1')]),
        _server('s2'),
      ]).resolveAllSourcesForGroup(_identity);

      // s1 is not offline — it is simply not in the profile any more.
      final result = await resolverFor([_server('s2')]).resolveAllSourcesForGroup(_identity);

      expect(result.items, isEmpty, reason: 'a server that left the profile cannot serve what it was cached as having');
      expect(result.coverage.expectedServerIds, {'s2'});
    });

    // A18: a server that left and came back under a new id is, to everything
    // here, simply a server the profile did not have when the miss was
    // cached. A negative may only speak for the servers it actually asked.
    test('A18: a complete negative is not replayed over a server the profile has since added', () async {
      await resolverFor([_server('s2')]).resolveAllSourcesForGroup(_identity);

      final readded = _server('s9', matches: [_item('s9')]);
      final result = await resolverFor([_server('s2'), readded]).resolveAllSourcesForGroup(_identity);

      expect(
        (readded.client! as _MatchingClient).lookups,
        1,
        reason: 'the new server was never asked by the cached run, so it cannot inherit its answer',
      );
      expect(result.items.map((i) => i.serverId), ['s9']);
      expect(result.coverage.checkedServerIds, {'s2', 's9'});
    });

    test('A18: a negative still stands over a narrower profile than it was cached for', () async {
      await resolverFor([_server('s1'), _server('s2')]).resolveAllSourcesForGroup(_identity);

      final remaining = _server('s2');
      final result = await resolverFor([remaining]).resolveAllSourcesForGroup(_identity);

      expect(
        (remaining.client! as _MatchingClient).lookups,
        0,
        reason: 's2 was asked in the cached run and answered nothing; losing s1 does not unlearn that',
      );
      expect(result.items, isEmpty);
      expect(result.coverage.isComplete, isTrue);
      expect(result.coverage.expectedServerIds, {'s2'});
  ...
  group('A19: expected-server denominator', () {
    ...
    test('an expected server that is online but never answers is unchecked, not absent', () async {
      final servers = eligibleSourceServers(
        ...
        clientFor: (id) => id.value == 's1' ? _MatchingClient(matches: [_item('s1')]) : _MatchingClient(throws: true),
        isOnline: (_) => true,
        authErrorServerIds: const {},
      );

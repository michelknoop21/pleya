...
void main() {
  group('eligibleCatalogLibraries', () {
    ...
    test('B4: a series-only profile has no movie libraries, and a Films query answers empty rather than guessing', () {
      final result = eligibleCatalogLibraries(
        libraries: [_library('shows', serverId: 's1', kind: MediaKind.show)],
        kind: MediaKind.movie,
        isServerVisible: (_) => true,
        hiddenLibraryKeys: const {},
      );
    ...
    test('B5: a movies-only profile has no show libraries, and a Series query answers empty the same way', () {
      final result = eligibleCatalogLibraries(
        libraries: [_library('movies', serverId: 's1', kind: MediaKind.movie)],
        kind: MediaKind.show,
        isServerVisible: (_) => true,
        hiddenLibraryKeys: const {},
      );

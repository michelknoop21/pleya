...
void main() {
  group('hoofdstuk 13.2 representative watch state', () {
    ...
    group('G7: runtime compatibility gate', () {
      ...
      test('an eliminated cut no longer switches off the progress tiers', () {
        // The gate asks whether progress may be reconciled between the
        // candidates still in the running, so it has to be read on the
        // survivors. Read on the whole group instead, a year-old extended
        // edition that tier 2 has already eliminated kept deciding that two
        // theatrical memberships could not be compared: the winner then fell
        // to map iteration order, and the card drew a watched tick over half
        // an hour of real progress.
        final now = DateTime.now().millisecondsSinceEpoch ~/ 1000;
        final watched = _watched(server: 'b', lastViewedAt: now - 120);
        final inProgress = _source(server: 'a', lastViewedAt: now - 120, viewOffsetMs: 30 * 60 * 1000);
        final staleExtended = _source(
          server: 'c',
          durationMs: 3 * _hour,
          lastViewedAt: now - 365 * 24 * 60 * 60,
          viewOffsetMs: 20 * 60 * 1000,
        );

        // Iteration order deliberately puts the watched source first, so
        // "iteration order stands" and "the progress tier ran" differ.
        final state = selectRepresentativeWatchState({'b': watched, 'a': inProgress, 'c': staleExtended});

        expect(state.representativeSourceKey, 'a', reason: 'tier 3 runs once the incomparable cut is out');
        expect(state.hasActiveProgress, isTrue);
        expect(
          state.runtimesDiffer,
          isTrue,
          reason: 'the group really does hold two different cuts — that fact is unchanged',
        );
      });

      test('D12: the gate is kind-agnostic — an extended-cut episode is exactly as incompatible as a film', () {
        // runtimesAreComparable/selectRepresentativeWatchState read only
        // durationMs; nothing here special-cases MediaKind.episode, so the
        // same "a higher raw offset on an incompatible cut never wins"
        // guarantee G7 already proves for films has to hold for an episode
        // that has, say, an extended special-edition cut on one server and
        // the plain broadcast cut on another.
        MediaItem episode({required String server, required int durationMs, int? viewOffsetMs}) => MediaItem(
          id: 'ep',
          backend: MediaBackend.plex,
        ...
        final extendedCut = episode(server: 'a', durationMs: 90 * 60 * 1000, viewOffsetMs: 80 * 60 * 1000);
        final broadcastCut = episode(server: 'b', durationMs: 45 * 60 * 1000, viewOffsetMs: 40 * 60 * 1000);

        expect(runtimesAreComparable(extendedCut, broadcastCut), isFalse);
        final state = selectRepresentativeWatchState({'extended': extendedCut, 'broadcast': broadcastCut});
        expect(state.runtimesDiffer, isTrue);
        expect(
          state.representativeSourceKey,
          'extended',
          reason: 'iteration order, not the longer cut\'s bigger raw offset',
        );
      });
    });
    ...
    group('tier 4: the remembered choice', () {
      test('breaks a tie every earlier tier judged equal', () {
        ...
        expect(selectRepresentativeWatchState({'a': a, 'b': b}, preferredSourceKey: 'b').representativeSourceKey, 'b');
      });

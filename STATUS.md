...
- [ ] **`ScrollReveal` kan content onzichtbaar laten op de landingspage**: `lib/components/ScrollReveal.svelte` zet `opacity: 0` plus een `translateY` en wacht op een `IntersectionObserver` die de inline style pas bij hydratie terugdraait. Vuurt die observer niet, dan blijft de sectie permanent onzichtbaar terwijl hij wel ruimte inneemt. Aangetoond met `pw screenshot` op een viewport van 1440×3000: alles onder de hero (`Screenshots`, `Features`, `FAQ`) blijft leeg, ook na 2,5 seconde wachten, en de gebouwde HTML draagt letterlijk `style="opacity: 0; transform: translateY(24px)"`. Raakt elke omgeving zonder werkende observer: een deel van de crawlers, screenshot- en previewtools, en alles met JavaScript uit. Geen cosmetisch probleem, want het gaat om de volledige inhoud van de landingspage onder de vouw. `/docs` omzeilt het al met een CSS-only `fade-in-up`; de landingspage is bewust niet aangeraakt om die wijziging niet met het documentatiewerk te vermengen. Kleinste afdoende fix is waarschijnlijk een `<noscript>`-override plus een `IntersectionObserver`-fallback die na een timeout gewoon zichtbaar maakt.

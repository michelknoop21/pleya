...
- [ ] **31 schermafbeeldingen voor de handleiding: negen staan er, 22 nog niet.** Geleverd op 2026-08-17 uit de simulators: `signin-choice`, `jellyfin-connect`, `home-hero`, `home-rows`, `player-controls`, `downloads-manage`, `search-results`, `search-tv` en `logs-screen`. Live op `/docs`, en de plaatshouder is daar verdwenen. Vier bijschriften zijn aangepast aan wat het beeld werkelijk toont in plaats van andersom; de afwijkingen staan in `docs/manual/SCREENSHOTS.md`. Van de resterende 22 zijn er zeven niet te maken met de huidige bronnen, en dat is geen kwestie van tijd: `demo.pleya.app` heeft één bibliotheek met films, dus alles met een serie valt weg (`sync-rules`, `detail-show`, `detail-episodes`), Requests vraagt een Jellyseerr-server, Live TV een tuner, `player-scrub` een vastgehouden druk op een echte Siri Remote, en `my-pleya` een profiel dat niet `applereview` heet. De rest is gewoon werk: de macOS-schoten, de bibliotheekschoten op de iPad, het sporenmenu en de instellingen. Het gereedschap en de valkuilen staan nu in `SCREENSHOTS.md`, inclusief dat `sips -c` vanuit het midden snijdt en dat de simulator eerst op Engels moet.
- [ ] **Add to Watchlist faalt op een Jellyfin-only verbinding**: op de demoserver geeft de knop op een filmdetail "Could not update your watchlist", en de favorieten van het account blijven leeg. Nagemeten buiten de app om, via `POST /Users/AuthenticateByName` en daarna `Filters=IsFavorite` op de Jellyfin-API: nul items, dus het schrijven bereikt de server niet. Twee keer gezien, op de iPhone en op de iPad, met build 221. Hierdoor is `watchlist-grid.png` niet te maken. Dit hoort bij de kijklijst-blocker hierboven, maar het is een concreet reproduceerbaar geval in plaats van een openstaande vraag.

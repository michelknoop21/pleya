#!/usr/bin/env node
// stop-hook.mjs : Claude Code Stop hook for the unlazy-safe skill.
//
// Structurally blocks ending the turn while the gate files contain unmet
// gates. Zero tokens: this is a file scan, not a model call. It executes
// nothing.
//
// Behavior:
//   - No gate files in cwd            -> allow (skill not active here)
//   - All gates met                   -> allow
//   - Nothing unmet but some abandoned-> allow, with a reminder that the
//     surrendered criteria must be named in the report
...
// State lives in .unlazy/state.json (legacy: .unlazy-hook-state.json in cwd).
//
...
for (const file of files) {
  ...
  const flush = () => {
    if (!cur) return;
    if (abandoned.has(cur.id)) { surrendered.push(cur.id); cur = null; return; }
...
if (!unmet.length) {
  // Nothing left to work on. Abandoned is not met: allow the turn to end, but
  // make sure the surrendered criteria cannot quietly read as delivered.
  if (surrendered.length) {
    console.log(JSON.stringify({
      systemMessage: `unlazy: ledger is terminal but NOT complete: ${surrendered.length} abandoned gate(s) (${surrendered.slice(0, 4).join(", ")}). Name each one and its reason in your report, and do not call the task done.`,
    }));
  }
  process.exit(0);
}
...
const stateDir = join(cwd, ".unlazy");
const statePath = existsSync(stateDir)
  ? join(stateDir, "state.json")
  : join(cwd, ".unlazy-hook-state.json");
...
console.log(JSON.stringify({
  decision: "block",
  reason: `unlazy: ${unmet.length} gate(s) unmet: ${list}. Work the next unchecked gate (gate-check.mjs --list prints its CHECK command; run it yourself, then --record the output), or add "ABANDON: <id> <reason>" if one is genuinely impossible. Done means every box checked with evidence.`,
}));
process.exit(0);

...
| SYS-3e | Na de SYS-3c-fix resten twee aanroepers van `TvBrowseRailLayout.scaleForSize` in `media_detail_screen.dart` die nog steeds de doos lezen in plaats van `TvLayoutConstants.scaleOf`: `railBottomPaddingForScale(TvBrowseRailLayout.scaleForSize(MediaQuery.sizeOf(context)))` op regel 4011 (bepaalt `initialForegroundBottom`, dus waar het scherm zijn voorgrondinhoud tegenaan laat lopen) en `TvBrowseRailLayout.scaleForSize(size)` in `_estimateTvDetailRailHeight`/`_estimateTvDetailEmptyRailReserveHeight` (regel 4800 en 4805, met `size = constraints.biggest`, bewust de doos volgens het commentaar op regel 3960-3966, bedoeld voor reserveringswiskunde binnen de eigen box). Sinds de SYS-3c-fix rendert `TvBrowseRail` zelf zijn bodempadding en radii op paneelschaal; deze twee plekken reserveren nog op doosschaal. Op de geneste route (kortere doos dan paneel) reserveert het scherm dus minder ruimte dan de rail werkelijk inneemt: een nieuwe mismatch, in de tegenovergestelde richting van de oorspronkelijke SYS-3c-bug. Niet gefixt in TV3 Task 7, buiten de scope van de eenregelige SYS-3c-fix | OPEN | n.v.t. | Gevonden tijdens de SYS-3c-fix (TV3 Task 7) met `grep -rn 'TvBrowseRailLayout.scaleForSize' lib/` na de fix. Toegewezen aan TV3 (zelfde eigenaar als SYS-3c, `media_detail_screen.dart`) |

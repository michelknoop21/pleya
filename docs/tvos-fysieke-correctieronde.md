...
| DET4 | Seizoenchips op seriedetail: de niet-geselecteerde labels renderen met een verschoven of dubbele glyphrij (tvOS-simulator, House, 13 september). Reproduceerbaar na een herstart, niet in de hero-artwork op dezelfde hoogte en niet in de gedimde topnav-labels, dus geen schermscheuring en geen algemeen probleem met gedimde tekst. Beperkt tot `_TvSeasonChip` in `lib/screens/media_detail/tv_season_chips.dart` (niet `lib/screens/tv/`, dat pad bestaat niet) | HARDWARE ONLY | n.v.t. | TV3 Task 1 (19 september): de geometrie-hypothese uit de preflight-rij hierboven (~7 logische pixels tekort door twee niet-meegetelde borders) is headless getoetst met een nieuwe test in `media_detail_screen_test.dart` ('DET4: a season chip fits inside the band the row reserves for it'), opgebouwd met dezelfde `_FakeMediaServerClient`-opzet als de bestaande seizoentest. Gemeten: `bandHeight=32.7`, `chipHeight=32.7`, gelijk, test groen. Kanttekening bij die meting: een `RenderBox` kan per contract nooit groter worden gerapporteerd dan de `maxHeight` die zijn ouder opgeeft (`BoxConstraints.constrain` klemt af zonder assertie), dus deze test kan een kandidaat-A-squeeze die netjes wordt afgeklemd niet van een werkelijk sluitende meting onderscheiden: beide geven `chipHeight == bandHeight`. Toch is groen hier het voorgeschreven, geldige resultaat (zie Task 1-brief stap 3) en kandidaat A wordt op dat bewijs losgelaten. Overige vier kandidaten afgelopen met de brief's grep-stappen: transforms vallen af (`disableScale: true` op de chip dwingt `Transform.scale` naar exact 1.0, `focusable_wrapper.dart:682`); opacity valt af op compositielaagniveau (kleuren lopen via `withValues(alpha:)`, geen `Opacity`-widget, `tv_season_chips.dart:167-171`) maar het kleur-/gewichtsverschil (`mono.textMuted` w500 tegen `mono.text` w600) blijft de enige echte visuele as tussen geselecteerd en niet-geselecteerd; tekststijl valt af als hoogtebron (de kale `TextStyle` mergt met `DefaultTextStyle`, alleen `fontWeight` wijkt af, en de headless meting bevestigt gelijke hoogte); animatie valt af (`AnimatedContainer(duration: mono.fast)` animeert achtergrond/rand/radius, maar een volledige app-herstart reproduceert de melding, dus geen halfweg gestopte transitie). Blijft over: Impeller's glyphatlas-compositie voor `mono.textMuted` op een transparante achtergrond tegen `mono.text` op een dekkende, een vraag die de Skia-testbackend principieel niet kan beantwoorden. Sluitende meting nog open: seriedetail van een show met minstens drie seizoenen op een tvOS-build met de SHA van deze branch, een foto van de chiprij met de ring op de eerste chip, en dezelfde foto na een app-herstart; `scripts/tvos_sim.sh shot` geeft hooguit een eerste indicatie en sluit de bevinding niet. Geen codewijziging in `lib/` in deze ronde |

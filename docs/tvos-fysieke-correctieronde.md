...
| DET4 | Seizoenchips op seriedetail: de niet-geselecteerde labels renderen met een verschoven of dubbele glyphrij (tvOS-simulator, House, 13 september). Reproduceerbaar na een herstart, niet in de hero-artwork op dezelfde hoogte en niet in de gedimde topnav-labels, dus geen schermscheuring en geen algemeen probleem met gedimde tekst. Beperkt tot `_TvSeasonChip` in `lib/screens/media_detail/tv_season_chips.dart` (niet `lib/screens/tv/`, dat pad bestaat niet). Root cause onbekend; mogelijk een Impeller-artefact dat alleen in de simulator optreedt | OPEN | | TV3-preflight (19 september, `862255cc`): `ls lib/screens/tv/tv_season_chips.dart` geeft "No such file or directory", `grep -rln '_TvSeasonChip' lib/` wijst naar `lib/screens/media_detail/tv_season_chips.dart` (een `part of '../media_detail_screen.dart'`), en die file heeft sinds 12 september geen commit gehad. Geometrie-hypothese (nog niet gemeten, dat is Task 1): `_tvDetailSeasonChipContentHeight` meet alleen `'Mg'` plus verticale padding; de chip zelf tekent `border: Border.all(width: 1)` (`tv_season_chips.dart:195`), en `FocusableWrapper` draait standaard `FocusIndicatorMode.ring` zonder `focusShapeBorder`, dus zijn `AnimatedContainer` krijgt `FocusTheme.focusDecoration` met `border: Border.all(width: focusBorderWidth)`, `focusBorderWidth = 2.5` (`focus_theme.dart:10`, `focusable_wrapper.dart:660-671`). Een `BoxDecoration`-border telt via `decoration.padding` mee in de layoutmaat, ongeacht `strokeAlign`, dus dat is 2 (1px × 2 zijden) + 5 (2,5px × 2 zijden) = ongeveer 7 logische pixels die de `SizedBox`-reservering niet meet. Schaalwaarneming: `media_detail_screen.dart:4142` bouwt de rij met `detailScale`, `media_detail_screen.dart:4800` reserveert er ruimte voor met `TvBrowseRailLayout.scaleForSize(size)`, twee verschillende schalen voor hetzelfde blok, zelfde familie als SYS-3a/SYS-3c |

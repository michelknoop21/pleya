...
| RAIL2 | Op Home lijkt de focus in een rail bij herhaald RIGHT, en volgens Michels aanvulling ook bij LEFT, af en toe een positie over te slaan en meteen een aantal tegels verder te landen (Michel, hardware, build 272, log `h6ocl`: "Ik heb ook af en toe bij home in de rails met naar rechts klikken meerdere malen dat hij soms een positie lijkt over te slaan dus gelijk naar een aantal items verder schiet. Ook bij left lijkt het te gebeuren"). `h6ocl` en `ijqxp` (build 280) gaven alleen timing/context (zie "RAIL2, classificatie op de twee beschikbare logs"): geen `uipress`-identiteit. Closure-run gedraaid op 15 september 2026, build 281 (`a7d5d3c7`), log `8x94u`: drie onafhankelijke fantoompaar-instanties (arrowRight ×2, arrowLeft ×1, hold ≤40 ms, gat ≤80 ms), alle drie `same-uipress`. Zie "RAIL2, closure-run op build 281". Formele status **ENGINE_DUPLICATE bevestigd** (zijdeur 4, dubbele swizzle-hop dispatch op `.ended`, `docs/tvos-remote-input-authority.md` §3 en §6): een engine-lifecyclepatch is volgens de beslisregel gerechtvaardigd, maar nog niet gebouwd | ENGINE_DUPLICATE CONFIRMED, fix niet gebouwd | n.v.t. |
...
§3). Een engine-lifecyclepatch is gerechtvaardigd. Er is in deze sessie geen patch gebouwd: de
opdracht was de meting, niet de fix.

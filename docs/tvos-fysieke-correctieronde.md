...
| OFF2 | Offline-tab-routingbug: `_handleOfflineStatusChanged` overschrijft de zojuist gekozen offline-tab | FIXED, hardware open | `0bda71a9` | Nameting bij de fix wijkt af van de oorspronkelijke `/code-review`-hypothese uit de OFF-1-ronde: niet de `displaced`-tak van `_syncTvDestinations()`, die raakt op TV nooit een Mijn Pleya-sectie (`displaced.tab` komt alleen uit `TvNavigationCoordinator`'s eigen root-destinaties). De echte oorzaak zit in `_selectTab`'s `isUserInitiated`/`previousTab != tab`-guard zelf: Bibliotheken/Kijklijst/Aanvragen/Downloads/Instellingen leven op TV allemaal onder de ene root-tab `NavigationTabId.myPleya` (hoofdstuk 18.2), dus `_currentTab` verandert nooit tussen die secties en de guard vuurt nooit. Praktisch effect, wél zoals de oorspronkelijke rij beschreef: kies je Kijklijst terwijl je nog op het auto-geopende Downloads-scherm stond, dan zag `_handleOfflineStatusChanged` bij het weer online komen nog de oude `_autoSwitchedToDownloads`-latch en herstelde die de tab van vóór het offline gaan, over de sectie heen die de gebruiker net zelf koos. Fix: `_openTvMyPleyaSection` maakt de latch zelf leeg via de nieuwe `shouldCancelAutoSwitchedToDownloads`, naar het patroon van `shouldRenderMainScreenOffline`. Negatieve controle: de nieuwe test faalde tegen een stub die het huidige (ontbrekende) gedrag nabootste, groen na de echte implementatie; `flutter analyze` en de volledige testsuite op deze worktree gaven geen nieuwe falers (80 pre-existente golden-/overige falers, stuk voor stuk in bestanden die deze wijziging niet raakt). Regressiescenario `tvos.home.hero-return-from-route` gedraaid op verzoek omdat deze wijziging gedeelde TV-shellcode raakt: eerst FAILED op een build zonder de toen nog niet gemergede HERO7-fix (exact dezelfde assertie op regel 53 als de HERO7-rij hierboven), na de rebase op de nu wél gemergede HERO7 opnieuw gedraaid en PASS, dus geen nieuwe regressie door MOC-23/OFF2 |

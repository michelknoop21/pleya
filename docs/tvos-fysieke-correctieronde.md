...
| TS1 | Top Shelf: Play op de remote moet hervatten en een klik de detailpagina openen; extra rij Nieuw toegevoegd. | CODE DONE · SIM/HARDWARE OPEN | `c307359c` | Play stuurt `action=play`, klik `action=open`, links zonder action houden het oude afspeelgedrag (Android Watch Next). Tweede sectie uit de al geladen Home-films en -series, op addedAt, max 10, zonder dubbelingen met Verder kijken. Unit- en kanaaltests groen, tvOS-simulatorbuild groen. Top Shelf-beeld en Play/klik op echte hardware zijn nog niet bekeken: de simulator toont de Top Shelf pas als Pleya in de bovenste rij staat. |

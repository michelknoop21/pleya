...
| DBL1 | Eén pijldruk geeft vaak twee stappen, in de topnav, de infodropdown, de speler en instellingen (Michel, Apple TV 4K, Siri Remote, 25 sep 2026, log `oc8pw`, build 303 `8c79abe`). Trace op de bijgewerkte `scripts/tvos_press_trace.sh`: tien keer `NATIVE-BOUNCE` (negen right, één up; left en down nul op 79 drukken), vier `RE-TAP(uikit-began)`, nul `engine-synth`. Voorbeeld 23:22:38.574: echte right van 136 ms, 20 ms na de ended een tweede `phase=0`/`phase=3`-paar van 20 ms, elk met een eigen `native press=`-regel. Station 1: UIKit levert zelf een tweede levenscyclus. Uitgesloten: zijdeur 4 (419 fase-regels, nul herhaalde fasen; de RAIL2-dedupe zou een herhaling bovendien afvangen), `tapIfMissingKeyDown` (elke keydown heeft zijn eigen began) en zijdeur 1 (geen kanaalbericht). Het eerdere oordeel `same-uipress` zegt niets: UIKit hergebruikt één `UIPress` per druktype (hash `62775` voor elke right), zie `docs/tvos-remote-input-authority.md` §4a. Open is of de remote (contactdender rechts) of tvOS (touchoppervlak) de tweede druk maakt. Een filter zou een tijdvenster moeten zijn, wat de regels verbieden (build 254), dus geen fix. Diagnostiek toegevoegd: `ts=` (`UIPress.timestamp`) in `tvos_press_diag`, vlag `NATIVE-BOUNCE` en oordeel `uikit-began`/`engine-synth` in de trace. SCRUB1 (tussen 303 en main) raakt alleen de scrubmodus en verandert dit niet. Volgende stap, HARDWARE ONLY: debug-log op build 304 of 306 volgens het rapport, plus twintig keer right op het tvOS-beginscherm met dezelfde remote | HARDWARE VALIDATION PENDING | `043d4ff0` (diagnostiek, geen fix) |

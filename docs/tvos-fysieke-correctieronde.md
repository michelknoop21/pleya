...
| FILT1 | Het catalogus-filterpaneel verliest sinds `8a6d1b52` een categorie onder de vouw zonder enige aanduiding. `TvCatalogLayout.filterZoneRows = 6` is in zijn eigen doccommentaar verantwoord als "hoger dan een volledige rail van **vijf** categorieën"; de audio-taalsectie maakte er zes van en het getal is niet meegegroeid. Gemeten op de vier filterpanelgoldens: rijafstand 50 px, vijf rijen in beeld, zes rijen vragen circa 294 px tegen een zone van circa 254. In `tv_catalog_filter_panel_multi` valt daardoor Status weg terwijl die een actieve telling "1" draagt, dus het enige teken dat dat filter aanstaat staat buiten beeld. Bereikbaar blijft het: de rail is een `SingleChildScrollView` en focus scrolt hem terug. Maar `_buildOptions` geeft zijn lijst wél `_FadingEdges` met het argument dat een vlakke afsnijding "leest als een renderfout in plaats van als 'hier gaat het verder'", en `_buildRail` kreeg die behandeling niet. Twee richtingen: `filterZoneRows` naar 7, of de rail dezelfde fade geven. Gevonden door een onafhankelijke review tijdens de GOLD3-ronde | OPEN | n.v.t. |
| GOLD4 | `test/goldens/tv_media_source_picker_golden_test.dart` tekent de Pleya Server-badge als een lege, wél ingesprongen plek. `BackendBadge` kan `pleyaServer` (`lib/widgets/backend_badge.dart`) via `Image.asset('assets/branding/pleya_logo.png')`, maar dit testbestand doet geen enkele `precacheImage`, terwijl `test/goldens/backend_badge_golden_test.dart` precies die PNG wél voorlaadt en in zijn commentaar uitlegt waarom. Geen productdefect: op een toestel laadt het logo gewoon. Wel een blinde vlek, want de goldens leggen nu een lege badge vast en een echte regressie daarop maakt ze niet rood. Repareren vraagt een `precacheImage` in de harness en daarna een nieuwe runner-regeneratie van de zeven bronkiezerbeelden. Gevonden door een onafhankelijke review tijdens de GOLD3-ronde | OPEN | n.v.t. |

...
| DENS1 | F-TV1, dichtheidsaudit (`docs/density-audit-2026-09.md`): de gedeelde `showOptionPickerDialog` (`lib/utils/dialogs.dart`, gebruikt door o.a. `media_context_menu.dart`, `record_options_sheet.dart`, `live_tv.dart`) sizede paneel en rijen met rauwe literals die op TV alleen de globale 1,85x-wrapper zagen, nooit `TvLayoutConstants.scaleOf`, zo'n 18% ruimer dan een getokend paneel bij dezelfde nominale waarde. Elke literal gaat nu door `scaleOf` op TV (`scale` is 1,0 elders, dus geen wijziging buiten TV) | FIXED | `792da906` |

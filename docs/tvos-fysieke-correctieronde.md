...
| VIS-0925-C | Instellingenkaart oogt als een tabel (buitenrand plus scheidingslijnen), `_languageSelector` mist `SettingRowFocus`, density-labels in hardcoded grijs, Switch-track verdwijnt in Light, lege `_ProfileHeader`-balk op Mijn Pleya. | FIXED, Verify-sim PASS, hardware open | `d135b813`, `6f744136`, `ef9c3de6` | Rijfocus in `test/screens/settings/settings_row_focus_test.dart`, Switch in `test/theme/switch_visibility_test.dart`. Sim: `tvos.settings.appearance`, `tvos.glass.topnav-light`, `tvos.my-pleya.sections`. Review-fixronde: het switchthema geldt alleen op de TV-instellingenrijen (via `TvSettingsDensity`); selected en disabled houden de Material-standaard. |
...
| VIS-0925-E | Hero in Light: de scrim is een witte waas over 60% van het beeld; CTA-ring in `tk.text` op 4 pt afstand geeft een zwart-witte dubbele rand; titel, CTA en eerste railtegel staan op drie x-posities. | FIXED, Verify-sim PASS, hardware open | `22e94b0e`, `b7a743e2`, `a5bb92f7`, `d23d5fa3` | Synopsis 4,5:1 over de hele kolom in `test/widgets/tv/tv_hero_scrim_light_test.dart`, x-lijn in `tv_content_feed_test.dart`. De eerste versie zette de hero op 83,34; na de review weer op 75,48 (hieronder). Review-fixronde (besluit Michel D1, D3): hero-titel, CTA-vulling en railkop terug op de paginarand (75,48), alleen de tegelartwork springt een ringband in; CTA-ring met dezelfde vrije band als de categorierail. Light-hero even helder als Dark: de +0,08 is overal weg (ook gedimd), de Light-leesscrim volgt buiten de tekstkolom de donkere ramp (pixeltest rechter 40%). Scenario's `tvos.home.hero-cta-focus`, `tvos.theme.dark-hero-cta`. |

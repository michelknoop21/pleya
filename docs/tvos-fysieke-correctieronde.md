...
| BACK1 | Zichtbare terugknop die de afstandsbediening niet bereikt. **TV8, 23 september:** Verify-journey `tvos.detail.movie` groen: geen terugpijl op het filmdetail, Menu brengt de ring terug op de kaart. | FIXED, VERIFY/SIM CLOSED, hardware open | `f00e2fe` |
...
| LIB7 | Bibliotheken wordt bronbeheer: bladeren loopt via de catalogus met bronfilter, Collecties en Afspeellijsten worden eigen unified ingangen; DEC-092 accepted, mockup 27 goedgekeurd. Gebouwd als `TvLibrariesScreen` (states A/B/C/D), negatieve controle groen, Pleya Verify-scenario `tvos.my-pleya.libraries-bronbeheer` PASS op de simulator (lijst, actieblad, sprong naar Alle films met bronfilter). Niet meegenomen: Mappen bladeren (geen TV-foldernavigator bestaat, eigen workitem) en het verhuizen van Collecties/Afspeellijsten naar Mijn Pleya (dat is I7-child 15/18). `tvos.my-pleya.library-chooser` en `tvos.library.sort` testten de vervangen tabbladtaal en zijn in TV6 ingetrokken: de kiezer is vervangen door bronbeheer (`tvos.my-pleya.libraries-bronbeheer`), en sorteren zit op TV in de catalogusrail (`tvos.catalog.rail-sort-focus`). | CODE/SIM CLOSED, HARDWARE OPEN | `eb5de4c9` |
| LIB8 | Collecties en Afspeellijsten hebben onder `Jouw verzameling` eigen ingangen en overzichten in Mijn Pleya. Plex-collecties blijven per bibliotheek; Jellyfin BoxSets en playlists zijn volgens de huidige clients serverbreed en verschijnen eenmaal per server. Broncontext, bestaande detailroutes, gedeeltelijke fouten, retry en focusherstel zijn code-/widgetgetest. `tvos.my-pleya.sections` doorliep hub, Bibliotheken, Servers en Samen Kijken met compositorbeelden, maar de app verdween bij de Instellingen-snapshot; de fixture bevat geen Plex/Jellyfin-collecties of playlists. Nieuwe ingangen en volledige Back-keten missen daarom nog een geslaagde Verify-/hardware-run. | CODE DONE · VERIFY/SIM OPEN · HARDWARE OPEN | branch `codex/netflix-redesign-tranche-1`; `test/providers/personal_media_provider_test.dart`, `test/screens/tv/tv_personal_media_overview_screens_test.dart`, `test/screens/tv/tv_my_pleya_screen_test.dart`; `.build/pleya-verify/tvos-my-pleya-sections-1790189834111/` (FAILED na 11 screenshots) |
| NLX2 | Goedgekeurd Netflix-redesign vervolg: Uiterlijk en Home-indeling op actuele simulatorbeelden beoordelen en TV-presentatie verbeteren. Behoud iedere bestaande voorkeursleutel, platformvoorwaarde, rij-ID, volgorde-/zichtbaarheidsactie en Back-focus. De twee identieke `Recent toegevoegd / Zolder`-rijen tonen nu `Films · Zolder` en `Series · Zolder`; Home-indeling gebruikt het gedeelde TV-paginakader. Uiterlijk heeft op TV vier categorieën, compacte keuzerijen met actuele waarde en dezelfde voorkeurshandlers; hoverbediening verschijnt alleen op pointerdesktop. Menu herstelt focus op de oorspronkelijke instellingentegel. Hardwarecontrole blijft open. | IN PROGRESS · HOME+UITERLIJK SIM VERIFIED · HARDWARE OPEN | branch `codex/netflix-redesign-tranche-2`; `tvos.settings.home-layout` PASS met compositorbeelden in `.build/pleya-verify/tvos-settings-home-layout-1790193631774/`; `tvos.settings.appearance-categories` PASS met compositorbeelden en D-pad/Back-focus in `.build/pleya-verify/tvos-settings-appearance-categories-1790195446720/`; gerichte widget-/providertests groen; plan `docs/superpowers/plans/2026-09-23-netflix-redesign-tranche-2.md` |
| NLX3 | Goedgekeurd Netflix-redesign vervolg: Verbinding toevoegen en daarna de resterende vensters tegen actuele UI beoordelen en per scherm alle bestaande routes en acties behouden. De TV-picker toont nu alle zes bestaande routes in een gedeeld twee-kolomsraster, met profielcontext en behoud van de pointerlijst. D-pad bereikt ook de onderste optie; Menu herstelt de Servers-tegel. De actuele Jellyfin-formuliercapture toont nog een gedeelde volschermpresentatie: formulierrestyling en de andere routes blijven open. Simulator bewijst presentatie/navigatie; aanmelden en Siri Remote-touch vragen aparte live-/hardwarecontrole. | IN PROGRESS · PICKER SIM VERIFIED · FORMS OPEN · HARDWARE OPEN | branch `codex/netflix-redesign-tranche-2`; `tvos.settings.add-connection-grid` PASS met compositorbeelden en Jellyfin-annuleerpad in `.build/pleya-verify/tvos-settings-add-connection-grid-1790196441525/`; gerichte widgettest voor vijf/zes routes en nested-success-terugkeer groen; plan `docs/superpowers/plans/2026-09-23-netflix-redesign-tranche-2.md` |
...
| MYP1 | Regressiebewijs voor het Mijn Pleya-werk | CODE/SIM CLOSED, HARDWARE OPEN | `0ea45b25` | `tvos.my-pleya.regression` loopt in één keer door de zes secties die `catalog.mixed.v1` bereikt (Bibliotheken, Servers, Samen kijken, Over, Logs, Instellingen). Per sectie: Select opent hem, Menu landt terug op dezelfde tegel. Daarna gaat Menu vanaf de hub naar `nav.myPleya`. Kijklijst (WL2), Aanvragen (geen Seerr), Downloads (verborgen op Apple TV) en Activiteit (ACT1, PS-9) zijn op deze fixture niet bereikbaar en worden niet nagebootst. PASS op `0ea45b25` met schone `lib/`, bundel `.build/pleya-verify/tvos-my-pleya-regression-1790176336786`; de hub, Instellingen en de terugkeer naar de tegel en de pil zijn op de screenshots nagekeken. |
| ACT1 | Activiteit is niet te verifiëren | ACCEPTANCE GAP, eigenaar PS-9 (besluit 23 september), fake Tautulli-server klaar (`f72466f2`, `0aa71164`) | n.v.t. | zie de sectie hieronder voor het geactualiseerde onderzoek van 14 september: `isOwnerOrAdmin` blokkeert de tegel voor elk profiel dat uitsluitend via Pleya Server verbindt, los van de fixture |
...
| APP1 | Uiterlijk op TV (categorie-indeling uit `7e3af503`): LEFT vanuit de instellingenkolom bereikt de categorierail alleen vanaf een rij die op dezelfde hoogte staat als een categoriekaart. Vanaf "Titels onder posters" (de zevende rij van Weergave) gebeurt er niets, de ring blijft staan. RIGHT vanaf een categoriekaart landt op de rij ernaast en niet op de eerste rij van die categorie: vanaf Home op de auto-advance-rij in plaats van de clearlogo-rij. Gevonden door `tvos.settings.appearance` in de TV8-suite; het scenario loopt nu eerst terug naar de bovenste rij. Een expliciete LEFT naar de geselecteerde categorie en RIGHT naar de eerste rij (het patroon van de catalogusrail) lost beide op, maar dat is een ontwerpkeuze in een scherm dat net is opgeleverd | CODE/SIM CLOSED · HARDWARE OPEN | `bc41b859` TV9 (24 september): LEFT vanaf elke rij naar de actieve categorie, RIGHT vanaf een categorie naar haar eerste rij. Widgettests falen op de oude code; `tvos.settings.appearance` PASS zonder de zeven keer UP, screenshot `02b-hero-rows` nagekeken. |
...
| HERO8 | Een vaste statuscapsule toont `Bekeken`, `Bezig · <percentage>` of `Niet bekeken` vanuit de gezamenlijke `UnifiedWatchState`; widgettests dekken alle staten, uiteenlopende serverstatussen en vaste CTA-positie. De huidige `/v1`-fixture kan deze hero-statussen niet op een echte Plex/Jellyfin-slide tonen. | CODE DONE · VERIFY/SIM OPEN · HARDWARE OPEN | branch `codex/netflix-redesign-tranche-1`; `test/widgets/tv_hero_billboard_carousel_test.dart` |
| HERO9 | De Home-hero selecteert maximaal twaalf geschikte, unieke recente films in plaats van acht; volgorde, 90-dagenfilter en deduplicatie blijven gelijk. Selector-, provider-, Discover- en carrouseltests zijn groen. De huidige `/v1`-fixture mist releasedata voor twaalf hero-films. | CODE DONE · VERIFY/SIM OPEN · HARDWARE OPEN | branch `codex/netflix-redesign-tranche-1`; `test/services/unified_catalog/featured_selector_test.dart`, `test/providers/tv_home_projection_provider_test.dart`, `test/screens/discover_screen_tv_hero_test.dart` |
...
| LAND6 | Een lege landing verbergt de route naar een gevulde catalogus. Gesloten door de lege-staat op de Films-/Series-landing een `actionLabel`/`onAction` naar de complete catalogus mee te geven, dezelfde `_LandingMessage`-vorm als de bestaande error-staat. Negatieve controle (`test/screens/tv/tv_discovery_landing_screen_test.dart`) stond rood zonder de actie, groen erna; volledige `test/screens/tv/`-suite groen op deze commit op OFF-1 na (pre-existing baseline-faler, niet van TV2). De negen production-goldens onder `tv_discovery_landing_production_golden_test.dart` bleven identiek rood (8/9) tegen zowel de pre-fix- als de post-fixcode, dus geen regressie van deze fix. **TV8, 23 september:** Verify-journey `tvos.landing.empty-action` groen op `catalog.empty.v1`, na de fix in LAND6b: DOWN vanaf de pil Films zet de ring op "Alle films", Select opent Alle films met zijn eigen lege staat en Films blijft actief. | FIXED, VERIFY/SIM CLOSED, hardware open | `f4bb816f` |
| LAND6b | De actie op een lege Films- of Serieslanding (LAND6) was met de afstandsbediening niet te bereiken. `focusActiveTabIfReady` vroeg de focus aan voor de koptekstactie `_viewAllFocus`, maar in de lege en de foutstaat staat die kop niet in de boom; `canRequestFocus` is ook op een losgekoppelde node waar, dus de aanvraag bleef hangen en `_focusFirstRail` werd nooit bereikt. DOWN vanaf de topnav landde nergens. Gevonden door een review van de TV8-diff, bevestigd door `tvos.landing.empty-action`. Gesloten: de check leest nu `context != null`, en valt terug op de actie van de lege staat | FIXED, VERIFY/SIM CLOSED, hardware open | TV8 |
...
| OFF1 | Geen reconnect-affordance op TV | FIXED, hardware open | `a749bf18`, `587750af`, `main@83b8f5fc` | `MainScreen._buildTvShell()` gaf `TvRootShell`/`TvTopNavigation` geen `isOfflineMode`, `isReconnecting` of `onReconnect` mee, terwijl de rail en de mobiele bottom bar die al hadden. `TvTopNavigation` toont nu, alleen offline en met een callback, een reconnect-item links van de bestemmingen, zelfde icoon-/labelgedrag als de rail en dezelfde plaats die `NavRailDestination.reconnect` op de desktoprail heeft. Dat las destijds als "tussen de profielchip en Mijn Pleya" omdat offline toen alleen Mijn Pleya tekende; sinds `bf0dd7c3` staat Home er offline naast, dus Rechts vanaf het item landt op Home. De widget is daar niet voor veranderd, de walk-test wel. Negatieve controle: de nieuwe tests faalden op een compilefout tegen de ongewijzigde widget, nu groen (37/37 in `tv_top_navigation_test.dart`). Alleen deze bevinding, niet de volledige MOC-23-compositie (PB-12): dat is een aparte, veel grotere bouwronde die Michel expliciet niet voor deze ronde koos. Een onafhankelijke adversarial review (Codex) op `a749bf18` vond drie echte gaten, alle drie gefixt in `587750af`: het automation-ID ontbrak in `AutomationIds.catalog()`/de gegenereerde yaml, Select viel onbehandeld door tijdens reconnecten (`onSelect: null` i.p.v. een echte no-op), en de reconnect-pill kon de remote-focus meenemen als hij verdween zodra de servers weer online kwamen. Die laatste beslissing (`reconnectWasFocused && !isOffline`) is alsnog als pure helper `shouldRecoverTvTopNavFocusAfterReconnect` geïsoleerd en met drie unit-tests vastgelegd, naar het patroon van `shouldRenderMainScreenOffline`; een volledige MainScreen+TvRootShell-integratietest is bewust niet gebouwd (disproportioneel voor deze bevinding). Gemerged op main ondanks twee rode, van de branch losstaande CI-checks: zie OFF3 en OFF4 hieronder |
| OFF5 | Offline-tab-routingbug: `_handleOfflineStatusChanged` overschrijft de zojuist gekozen offline-tab | NIET REPRODUCEERBAAR | `8c9ce879` | `main_screen.dart:2143` (`_selectTab`) wordt onvoorwaardelijk opnieuw toegewezen zodra `_syncTvDestinations()` een `displaced`-tab teruggeeft, ook als de gebruiker net zelf naar Downloads/Watchlist navigeerde: `isUserInitiated: false` onderdrukt alleen de `_autoSwitchedToDownloads`-reset, niet de `_currentTab`-toewijzing zelf. Gevonden door `/code-review` tijdens de OFF-1-ronde, niet zelf op hardware herhaald. Praktisch effect (bewering van de review-agent): op een TV waar Downloads/Watchlist offline zichtbaar is, landt de offline-shell alsnog op het generieke Mijn Pleya-scherm i.p.v. op Downloads/Watchlist. Hoort bij MOC-23/OFF-2, niet in de OFF-1-branch gerepareerd. Heette tot 18 september ook OFF2, naast de dode-pills-bevinding; hernoemd in TV0 omdat één ID niet twee problemen kan dragen. | TV6, 23 september: de widgettest `test/screens/main_screen_tv_offline_destination_test.dart` mount `ProfileSessionScreen` met de echte `MainScreen` en loopt de keten offline → bestemming → herberekening. Downloads en Kijklijst openen op TV via `_openTvMyPleyaSection` onder Mijn Pleya, en `buildTvDestinations` haalt Mijn Pleya offline nooit weg, dus `updateConditions` verdringt niets. Drie tests (Android TV Downloads en Kijklijst, Apple TV Kijklijst) zijn groen op ongewijzigde code, heen en terug. Negatieve controle: de `next.contains(_active)`-guard in `updateConditions` weggehaald maakt 4 van 5 rood. Het praktische symptoom uit de review (landen op de kale Mijn Pleya-hub) bestaat wel, maar via een ander pad: zie OFF6. |
| OFF5b | Terug online zet `_handleOfflineStatusChanged` op Android TV het verdrongen tabblad (Films) terug zonder `_selectTab`, dus de topnav blijft Home oplichten boven de Films-landing | FIXED, hardware open | `8c9ce879` | Gevonden door de OFF5-test. Rood op ongewijzigde code (`Expected: TvDestinationId.movies, Actual: TvDestinationId.home`), groen na `_tvNav.syncToTab(_currentTab)` na `_syncTvDestinations()`. **TV7-correctie na review:** die fix zette alleen de balk goed. De terugzetting wees `_currentTab` nog direct toe, buiten `_selectTab`, dus `onTabHidden`, de Verify-routestatus en `navigation.tab_changed` bleven achter, en de focus in de balk verhuisde niet mee met de ring. Nu loopt de terugzetting via `_selectTab(…, isUserInitiated: false)` en volgt de focus als de remote in de balk stond; de test controleert ook de gefocuste pil en is rood tegen de TV6-code. Open productvraag: Apple TV zet Films na herverbinden niet terug, omdat de vlag aan de Downloads-autokeuze hangt en niet aan het verdringen van de bestemming. Apple TV raakt dit niet: Downloads is daar verborgen, dus er wordt geen herstel gewapend en de app blijft na herverbinden op Home. Dat verschil tussen de platforms bestond al en is niet gewijzigd. Geen Verify-scenario: de fixture heeft geen offline-staat en het pad bestaat alleen op Android TV, dat geen Verify-target is. |
| OFF6 | Een rebind na herverbinden wist de geopende Mijn Pleya-sectie: `binder.rebindActive()` → `invalidateNow` → `_invalidateAllScreens` → `_tvNav.clearNestedRoutes()`, en de kijker landt op de kale hub | CODE/SIM CLOSED · HARDWARE OPEN | `60247078` | Gevonden door de OFF5-test (TV6, 23 september). Treedt op wanneer `MainScreen` online komt zonder zichtbaar verbonden server, of na Opnieuw verbinden wanneer `shouldRetryActiveProfileBindAfterReconnect` waar is. Open vraag: moet een rebind voor hetzelfde profiel de geneste routes wissen? Niet gerepareerd in TV6. TV9 (24 september): een rebind van hetzelfde profiel wist geneste routes en focusgeheugen niet meer; verdwijnt de tegel van de open sectie, dan sluit alleen die sectie. MainScreen-harnastest faalt op de oude code; `tvos.my-pleya.regression` PASS. Het sluitpad heeft geen widgettest (de harness kent geen sectie die bij een rebind verdwijnt). |
| PROF1 | `nav.profile` opent op TV de generieke `ProfileSwitchScreen`-lijst (volle breedte, geen TV-inzet, mobiele stijl) en niet de poortcompositie van mockup 21; `TvProfileGate` verschijnt alleen met `requireSelection: true` | CODE/SIM CLOSED · HARDWARE OPEN | `bf01cb43` | Gevonden in de TV7-run van `tvos.profile.picker` (23 september). Open vraag: moet de kiezer vanuit de balk ook de TV-poort tonen? Niet gerepareerd in TV7. TV9 (24 september): `nav.profile` en Profiel wisselen openen `ProfileSwitchScreen(presentAsGate: true)`, de compositie van mockup 21 met het contract van de beheerlijst; Menu sluit de route. De eerste simulatorrun ving dat Menu op tvOS als toets binnenkomt; opgelost en met een toetstest vastgelegd. `tvos.profile.picker` en `tvos.auth.first-profile-handoff` PASS, screenshot `01-picker` nagekeken. |
...
| GOLD3 | De vierentwintig Linux-referenties die na de pin-reparatie rood stonden, zijn geregenereerd via `.github/workflows/goldens.yml` (run `35781164268`) en per beeld beoordeeld: bbox en maximale kanaalafwijking gemeten, uitsnede naast de referentie gelegd, oorzaak teruggezocht in de historie. Elk verschil hoort bij een geregistreerde ronde: MOC-11's `BackendBadge` op acht bronrijen (`968d794e`, referenties van 30 augustus), de audio-taalsectie op vier catalogus-filterpanelen (`8a6d1b52`, referenties van 6 september), DET6's tweede synopsisregel op twee detailbeelden (`4020547e`), VER3's `railLeadInset` op vijf discovery-beelden (`faff5ba0`, gemeten als exact `dx=+4` tegen `cardFocusRingGap * 0.85 = 4,25`), LAND7's rail-anker op de twee production-discovery-beelden (`1f69a855`, gemeten als 107 pixels verticaal), de herbouwde zoekkop van mockup 36 B plus SYS-4's lege-staat op `tv_shell_search` (`30aae55b`), en TOK2's `kSuccess` op de serverstip in `tv_shell_my_pleya_full` (`5fbe2c7b`, kern `0xFF3FBF5F` naar `0xFF3DD68C`). Alleen `tv_shell_home` is renderruis, maximaal 9 van 255 op alle drie de kanalen langs de randen van de skeletkaarten. Een onafhankelijke adversarial review van de vierentwintig corrigeerde twee van mijn eerste verklaringen: de serverstip was een tokenwijziging en geen antialiasing, en de twee production-beelden dateren van ná VER3 | FIXED | `4832403a` |
| FILT1 | Het catalogus-filterpaneel verloor sinds `8a6d1b52` een categorie zonder enige aanduiding. `TvCatalogLayout.filterZoneRows` stond op 6 en was in zijn eigen doccommentaar verantwoord tegen een rail van vijf categorieën; de audio-taalsectie maakte er zes van en het getal groeide niet mee. Gemeten aan de paneelgeometrie in plaats van aan de beelden: de rail kreeg een viewport van 254,15 px voor 293,75 px inhoud, `maxScrollExtent` 39,6. De weggevallen categorie is Status, en die verdwijnt aan de bovenkant en niet onder de vouw zoals de bevinding aannam, terwijl de Filters-capsule het paneel juist op Status opent en het telbadge het enige teken op het scherm is dat dat filter aanstaat. De twee voorgestelde richtingen bleken geen alternatieven maar twee helften. `filterZoneRows` op 7 zet `maxScrollExtent` op 0 en herstelt de eigenschap die de constante hoort te garanderen; zeven dekt de breedste rail die het paneel kan bouwen, want `TvCatalogFilterSection` heeft zes waarden en de rail noemt ze alle zes. De marge is wel dun, 3,75 px, dus de eigenschap wordt vastgehouden door een test die de scrollextent meet en niet door rekenwerk op de constanten: dat rekenwerk was juist de fout, want een gezette categorierij is hoger dan `optionRowMinHeight`. Een onafhankelijke adversarial review wees de tweede helft aan: `_zoneHeight` klemt op de beschikbare hoogte, dus onder ongeveer 555 logische pixels scrolt de rail alsnog, en `setForceTv` zet dit paneel op vensters die zo kort zijn. Die rail heeft nu dezelfde `_FadingEdges` als de optiekolom, met een eigen test op een te kort oppervlak; op het tvOS-canvas is die fade inert en verandert hij geen enkel beeld. Labels staan op `maxLines: 1`, dus geen locale kan een rijhoogte oprekken. Vijf filterpaneelreferenties opnieuw gegenereerd op de Linux-runner. Code, tests en CI zijn dicht; de goldens zijn per hoofdstuk 29 geen pixelwaarheid voor tvOS, dus de rij gaat pas naar VERIFIED na een echte Apple TV-ronde waarin het filterpaneel geopend is | FIXED, hardware open | `3ed8cdae` |
| GOLD4 | `test/goldens/tv_media_source_picker_golden_test.dart` legde de Pleya Server-badge vast als een lege maar wel correct ingesprongen plek. De oorzaak zit in het harnas en niet in `BackendBadge`: `Image.asset` decodeert in de engine-codec, en de testbinding draait die klok niet, dus de badge was nog niet geverfd toen het beeld werd genomen. De twee SVG-badges ernaast hebben dat niet, want `flutter_svg` decodeert in pure Dart; de gecommitte referentie laat de Plex-chevron en het Jellyfin-merk gewoon zien terwijl de PNG ertussen leeg is. Opgelost met `precacheImage` binnen `runAsync` in `_pumpPicker`, hetzelfde patroon dat `backend_badge_golden_test.dart` al gebruikt. Gemeten voor en na op dezelfde machine: van de acht bronkiezerbeelden verandert er één, `tv_source_picker_mixed_states`, binnen een vak van 14 bij 13 pixels; de andere zeven zijn byte-identiek. De schatting van zeven te regenereren beelden in de oorspronkelijke bevinding klopte dus niet, want `_sparse` is de enige `pleyaServer`-bron en komt in één geval voor. Het doccommentaar van het zusterbestand beweerde dat de SVG's de echte pauze nodig hebben; dat is met hetzelfde bewijs rechtgezet. Hardware is hier geen openstaande stap: het defect zat aantoonbaar in het golden-harnas en op een toestel laadde het logo altijd al | FIXED | `3ed8cdae` |
...
| PLR10 | De slaaptimer in het spelerpaneel toont "5minuten" en "1uur, 30minuten", zonder spatie; mockup 33 schrijft "15 minuten" | CODE/SIM CLOSED · HARDWARE OPEN | `a41b3643` | Gezien in de TV7-run van `tvos.player.panel` (23 september). Niet gerepareerd in TV7, dat bewijs levert en geen herontwerp. TV9 (24 september): de fout zat in de gedeelde `formatDurationTextual` (spacer leeg ook bij de volledige notatie); nu '5 minuten' en '1 uur, 30 minuten', unittest erbij. |
| PLR11 | Witte spelerknoppen (Volgende aflevering, Aftiteling overslaan) tonen zwarte hoekjes in de focusrand (Michel, 24 september). Twee oorzaken, gemeten: (1) `FocusableButton` tekent de ring om Material's tapvlak van 48 px (`materialTapTargetSize.padded` op iOS/tvOS) terwijl het witte oppervlak 44 px is, dus 2 px donker boven en onder en halvemaantjes aan de uiteinden; (2) `SkipMarkerButton` geeft ring en knop dezelfde `radiusSm`, maar de ring is een binnenrand van 2,5 px, dus de bochten zijn niet concentrisch en de zwarte schaduw van de knop valt in de wiggen. Op verzoek van Michel meegegeven aan de Liquid Glass-lijn (DEC-122), die spelerknoppen opnieuw vormgeeft | OPEN, overgedragen aan Liquid Glass | n.v.t. |
...
| WL3 | Regressie van WL1: `watchlist_screen_test.dart` "removing the card the remote is on leaves the remote on a card" faalt op `github/main` zelf (`6be0efca`), dus onafhankelijk van deze taak en niet door de merge veroorzaakt. `Expected: true, Actual: false` op "the slot is kept, so the card that slid up into the empty cell takes the ring". Gesloten: de verwijderde kijklijstkaart geeft de ring nu door aan de kaart die in de leeggevallen cel opschuift, in plaats van de ring te laten vallen. `test/screens/watchlist_screen_test.dart` is in zijn geheel groen (51/51). **TV8, 23 september:** geen Verify-journey mogelijk, zelfde grens als WL2 (`/v1` heeft geen kijklijst). | FIXED | `9963071c` |
...
| SEARCH3 | DOWN vanaf de zoekpil op Apple TV landde op de kaart die het dichtst bij het midden van de pil lag, bij twee resultaten de tweede in plaats van de eerste. `_handleTvKeyboardNavigateDown` probeerde eerst `focusInDirection(down)`, en vanaf een pil over de volle breedte kiest die geometrisch. Op Apple TV staat tussen de pil en de resultaten niets (geen mic, het inline toetsenbord alleen als fallback), dus de pil gaat nu eerst naar `TvSearchView.focusFirstResult`, het doel dat die methode al documenteerde. Gevonden door `tvos.search.results`, dat na de fix groen is | FIXED, VERIFY/SIM CLOSED, hardware open | TV8 |
| VIS1 | Visuele acceptatie TV8, Zoeken tegen 36 B: de zoekpil loopt in de app over de volle breedte, in 36 B tot ongeveer de helft; de telling ("2 resultaten") staat in de app tegen de bovenrand van de pil, in 36 B verticaal gecentreerd. Bewijs: `tvos.search.results`, screenshot `02-results.png`. Niet in TV8 gerepareerd: de breedte is een compositiekeuze die niemand heeft vastgelegd, geen regressie, en het register kent geen besluit dat de volle breedte goedkeurt | CODE/SIM CLOSED · HARDWARE OPEN | `a41b3643` TV9 (24 september): TvSearchPill op halve breedte van de inhoudskolom, op de rand van de resultaten, telling op de middellijn. `tvos.search.results` PASS met asserts op `tv.search.pill` (state `count`) en de grid eronder; screenshot `02-results.png` tegen 36 B nagekeken. |
| VIS2 | Visuele acceptatie TV8, seriedetail tegen 37 C: in 37 C staan de seizoenchips op dezelfde regel als de kop "Afleveringen", met het aantal afleveringen erachter; in de app staan de chips op een eigen rij boven die kop, zonder aantal. Bewijs: `tvos.detail.series`, screenshots `00-detail.png` tot en met `04-chip-3.png`. De MOC-10-rij legt de heroafwijking vast als productbesluit, deze niet. Niet in TV8 gerepareerd: de chiprij is sinds DET4 op hoogte doorgerekend, en een verplaatsing raakt die meting en de focusroute UP vanaf de rail | CODE/SIM CLOSED · HARDWARE OPEN | `a41b3643` TV9 (24 september): kop, seizoenchips en `N afleveringen · M bekeken` op één regel op de kopstrook van de seizoensrail; chips als gevulde pillen, de actieve wit. Eerste run liet de actieve chip op de focusring van aflevering 1 vallen en de rij tegen de actieknoppen; beide opgelost. `tvos.detail.series` PASS, screenshots `00-detail` tot en met `04-chip-3` nagekeken. |
| VER6 | `tvos.sidebar.collapse` test vervangen UI en is in de volledige tvOS-suite van TV8 rood: `sidebar.rail` bestaat op TV niet meer sinds de topnavigatie, en de tweede helft opent `screen.libraries`, dat LIB7 op TV heeft opgeheven. Het scenario is niet te repareren tot iets met dezelfde betekenis; de topbalk heeft zijn eigen journeys (`tvos.nav.walk`, `tvos.nav.focus-switches-destination`). Niet verwijderd, omdat DEC-081 dit bestand aanwijst als referentiegate voor de false-PASS-toets en `pleya_verify/mcp`-tests het bij naam verwachten. Intrekken vraagt een opvolger van DEC-081 met een nieuwe gate | CLOSED in TV9: scenario verwijderd, referentiegate is nu `tvos.nav.focus-switches-destination` (DEC-120), MCP-tests 25/25 | n.v.t. |
| VER7 | `tvos.player.osd`, `tvos.player.panel` en `tvos.player.zoom` zijn flaky op dezelfde build: in de TV8-suite en de herhaalruns samen groen en rood door elkaar (`panel` 1 van 3 rood in de laatste reeks). Oorzaak in de bundel: de fixtureclip duurt 2 seconden, en de speler sluit zichzelf ("Autoplay: no next episode, leaving the player") voordat de pauzedruk van het scenario aankomt. Een langere clip lost het op, maar `tvos.home.hero-return-after-playback` rekent erop dat de clip afloopt, dus dat is een fixturekeuze met twee kanten. Een run in de sequentiële suite zag ook één keer de automation-server wegvallen terwijl de app nog invoer verwerkte (`tvos.my-pleya.sections`, groen bij herhaling) | CLOSED in TV9: clip 15 s; osd, panel, zoom 3 van 3 groen; `hero-return-after-playback` wacht op het einde van de clip en is groen | n.v.t. |
| VER8 | `media-detail.episode-refresh` liep via Mijn Pleya > Bibliotheken > bibliotheek > raster. Sinds LIB7 is Bibliotheken op TV bronbeheer en opent Select op een bibliotheek een beheersheet, dus het scenario strandde op `library.items_loaded`. Het zat niet in de `tvos.*`-suite van TV8 en viel daarom niet op. Gevonden in TV9 (24 september). Nieuwe route via Home, zoals `tvos.detail.series`: `catalog.shows.v1` zet de serie nu in `recently_added`, het scenario opent haar vanaf de eerste rail en speelt de aflevering onder de ring af. | CLOSED in TV9: groen op de simulator | n.v.t. |
| DENS1 | Instellingenvensters, seriedetail en filmdetail voelen op Michels 77 inch tv "nog steeds enorm opgeblazen" (24 september). Gemeten tegen Apple's tvOS-HIG: een instellingenrij van 138 pt rond 26/22 pt tekst (4,5 rijen per scherm), een indextegel van 160 pt met een waarderegel van 19 pt (onder het minimum van 23 pt), een detailtitel van 88 pt (Title 1 is 76), een actieknop van 72 pt rond een label van 27 pt en een bronregel van 20 pt. Oorzaak: de klem van 0,85 op `scaleOf` maakt op Apple TV alles 1,57 keer zo groot. Fix volgens DEC-130: `TvHig` rekent in Apple's punten; rijen, tegels, categorieën en het detail-informatieblok in Body 29 / Caption 1 25 / Caption 2 23 / Title 1 76 pt, knoppen 60 pt. Onderweg gevonden: `tvos.settings.appearance-categories` was verouderd (verwachtte de ring bij openen op categorie 0, de pagina opent op de eerste rij zoals `tvos.settings.appearance` telt); het scenario gaat nu eerst LEFT, ook zonder DENS1 rood aangetoond | CODE/SIM CLOSED · HARDWARE OPEN | TV9 DENS1 | 
...
| DBL1 | Eén pijldruk geeft vaak twee stappen, in de topnav, de infodropdown, de speler en instellingen (Michel, Apple TV 4K, nieuwste Siri Remote met USB-C, 25 sep 2026, log `oc8pw`, build 303 `8c79abe`). Trace: tien keer `NATIVE-BOUNCE` (negen right, één up), vier `RE-TAP(uikit-began)`, nul `engine-synth`. Voorbeeld 23:22:38.574: echte right van 136 ms, 20 ms na de ended een tweede `phase=0`/`phase=3`-paar van 20 ms, elk met een eigen `native press=`-regel. Station 1: UIKit levert zelf een tweede levenscyclus. Uitgesloten: zijdeur 4 (419 fase-regels, nul herhaalde fasen), `tapIfMissingKeyDown` (elke keydown heeft zijn eigen began) en zijdeur 1 (geen kanaalbericht). `same-uipress` zegt niets: UIKit hergebruikt één `UIPress` per druktype, zie `docs/tvos-remote-input-authority.md` §4a. 26 sep, log `v5okk` (build 306 `35fd421`): zes `NATIVE-BOUNCE`, alle op right; een tweede remote van hetzelfde type geeft hetzelfde, en op het tvOS-beginscherm is één druk één stap, dus defecte hardware valt af. De passthrough-schakeling ook: de engine laat die alleen op Menu werken (`shouldPassMenuPressToSystem:`), en de eerste reeks (10:20:00.077-.382) heeft geen schakeling. Niet bewezen is wat UIKit tot de extra levering brengt. Kandidaten staan in de engine: `reportsAbsoluteDpadValues = true` op de micro gamepad (`setupTouchesWithGamepad:`) en de swizzle die bij een geclaimde druk de originele `sendEvent:` overslaat; in `Info.plist` staan sinds de import `GCSupportsControllerUserInteraction` en `GCSupportsMultipleMicroGamepads`. Het beginscherm onderscheidt ze niet, want de focus-engine daar kan extra drukken ook opvangen. Een filter zou een tijdvenster zijn, dus geen fix. Diagnostiek: `ts=` (`UIPress.timestamp`, `043d4ff0`), daarna de hardwarevelden `gcA`/`gcX`/`gcY`/`gcN`/`resp`/`gr` in `tvos_press_diag` en een `hw=`-oordeel per bounce in de trace (`click-held`, `no-click`, `click-toggles`). Volgende stap, HARDWARE ONLY: één debug-log op een build met deze commit, recept in het DBL1-rapport | HARDWARE VALIDATION PENDING | `043d4ff0`, `c3066951` (diagnostiek, geen fix) |
...
| HOOK1 | Een push vanuit een worktree liep tweemaal vast op een `pre-push`-hook die eerst zelf `docs: releasenotes bijwerken` committe en daarna de push afbrak, terwijl de hook in deze worktree sinds `4f9c69f2` juist adviserend is en niets meer schrijft. Oorzaak is niet de hook maar `core.hooksPath`: die stond op het absolute pad `/Volumes/SSD/Projects/PlexFlixNetwork/plezy-main/.githooks`. Die sleutel staat in de gedeelde `.git/config` en geldt dus voor alle vijfentwintig worktrees tegelijk, die daardoor allemaal de hooks draaien van die ene checkout, zoals die daar op schijf staan op de branch die er uitstaat. En daar zit de tweede helft: de hoofdcheckout heeft lokaal `main` op `6052e824` staan, ruim vóór `4f9c69f2`, dus het bestand dat werkelijk draaide was nog de oude schrijvende versie. Een onafhankelijke review haalde die volgorde eruit; de eerste formulering schreef de oude hook aan een commit toe die hem al vervangen had en maakte daarmee zijn eigen symptoom onmogelijk. Reproductie in een kale testrepo bevestigt de mechaniek aan beide kanten: met het relatieve `.githooks` draait elke worktree zijn eigen hook, met een absoluut pad draait overal het bestand van de hoofdcheckout, en `git config` vanuit een worktree schrijft die waarde ook nog eens in de gedeelde config. `scripts/setup_hooks.sh` zette altijd al het relatieve pad en repareert de sleutel dus door hem simpelweg te draaien; wat ontbrak was zichtbaarheid en een kloppende beschrijving. De waarschuwing in `check_hooks_installed.sh` noemt nu het gevolg bij een absoluut pad, inclusief de `~`-vorm die git zelf expandeert, en beschreef zelf nog het oude bijhoudgedrag van de pre-push hook; `setup_hooks.sh` beschreef dat ook nog, plus een pre-commit-gate die de unused-scans zou draaien terwijl die sinds `4f9c69f2` CI-only zijn. Lokale config rechtgezet naar `.githooks`, wat alle worktrees in een keer dekt. Blijft open en is geen codewijziging: de hoofdcheckout staat achter op `main` en moet bijgetrokken worden, anders blijft daar de oude hook op schijf staan | FIXED | `bbc4f003` |
| DENS1 | Volledige viewport- en informatiedichtheidsaudit per gebruikersoppervlak voor tvOS, iOS, iPadOS en macOS. tvOS moet 1080p- en 2160p-output apart bewijzen zonder fysieke 4K-pixels als extra logische layoutruimte te behandelen; iPhone, iPadOS en macOS krijgen elk hun eigen device-/venstermatrix, safe-area, invoer- en scrollcontract. | IN PROGRESS | n.v.t. | Auditregister: `docs/ui-viewport-density-audit.md`. Verse 4K-smoke PASS: compositor 3840×2160, root 1920×1080 @2×, Pleya-logical 1037,84×583,78. Gevonden acceptance-gaten: geen iPad-/oriëntatie-/Split View-matrix, geen macOS-resize en geen scroll/drag in Verify; bijbehorende surfaces blijven expliciet OPEN. |
| SCRUB1 | Scrubben in de speler: na een klik op de tijdlijn gaf elke veeg links of rechts hooguit één positie; Michel wil de cursor vrij over de balk kunnen bewegen en met een klik daar verder spelen. Oorzaak: in de scrubmodus liep de pan door de swipe-naar-pijl-omzetting van `AppleTvRemoteTouchService._moveTouch` (één pijl per 180 punten, hooguit één per 190 ms, en het hele gebaar gedempt zodra de systeempijl van tvOS eerst binnenkwam), en elke pijl was één vaste seekstap. | CODE DONE · HARDWARE OPEN | `7a8fa81f` | Tijdens de scrubmodus krijgt `DesktopVideoControls` de horizontale pan via `setScrubPanHandler`; een pan van 60 punten of meer claimt het gebaar, zodat de systeempijl niet meestapt. De native kant is ongewijzigd, de engine stuurde `move` met coördinaten al. Widget- en servicetests groen, simulatorbuild groen. idb stuurt alleen HID-toetsen en geen pan over het touchoppervlak, dus het gevoel van het scrubben is alleen op hardware te toetsen: speler, tijdlijn focussen, klikken (pauze), langzaam en snel vegen, klikken (verder spelen vanaf de cursor), en Menu (terug naar de oude positie). Afstemmen doe je met `_fastSpeed` en `_maxGain` in `lib/widgets/video_controls/helpers/apple_tv_scrub_pan.dart`, en met `scrubPanFullTravel` (1920 viewpunten per volle veeg) in `AppleTvRemoteTouchService`. Reviewronde 1 (fixcommit hieronder in de git-log, onderwerp "scrubgebaar opnieuw verankerd"): het gebaar wordt bij elke wissel van de scrub opnieuw verankerd, na commit of Menu met de vinger nog neer geeft de rest van die aanraking geen pijl meer, de gain rekent in viewpunten in plaats van de geschaalde schermbreedte (was 1,85 keer te hoog), de snelheid is een gemiddelde over 50 ms met een ondergrens van 8 ms, en een vroege systeempijl telt niet meer op bij de pan. Een verticale veeg verlaat de scrub zoals voorheen. Reviewronde 2 (onderwerp "basis bij de klik met de vinger neer"): klik je met de duim al op het oppervlak en veeg je in dezelfde aanraking door, dan ligt de basis vast bij de klik, zodat een systeempijl vóór de eerste 20 punten niet meer 10 s extra geeft; een klik gooit ook de opgespaarde rest onder de dode zone weg. Extra hardwarestap: klik met de duim op het oppervlak en veeg zonder optillen door, de cursor moet dan precies de veeg volgen, zonder sprong van één seekstap. |
...
**23 september 2026, TV6-besluit van Michel.** Het contract wacht op PS-9. Wie
Activiteit mag zien is een rechtenvraag, en die hoort bij het gebruikers- en
rechtenmodel van PS-9, niet bij een clientcapability die vooruitloopt op wat
de server straks over rollen zegt. `isOwnerOrAdmin` blijft ongewijzigd, het
protocol ook. MYP1 bewijst Mijn Pleya zonder de Activiteit-tegel. Blijft
`ACCEPTANCE GAP` tot PS-9 de rol levert.
...
| ID | Bevinding / verzoek | Status | SHA | Bewijs / volgende stap |
|---|---|---|---|---|
| UX23 | Netflix-redesign beoordelen vanuit de actuele GUI, Home behouden en alle functies meenemen. | IN PROGRESS, eerste tranche goedgekeurd | n.v.t. | Michel keurde op 23 september de vier richtingen goed en benadrukte dat dit nog niet alles is. [Actuele audit en verbeterplan](design-audits/2026-09-23-netflix/README.md) en [uitvoeringsplan tranche 1](superpowers/plans/2026-09-23-netflix-redesign-tranche-1.md). Auditwandeling op `6052e824` PASS, 24 simulatorbeelden. Collecties/Afspeellijsten blijven bij LIB7 en zijn bestaande vervolgeigenaar; pakketten B–F blijven open. |
| UX23-HERO | Meer hero-items zonder Home opnieuw te ontwerpen. | CODE DONE · VERIFY/SIM OPEN · HARDWARE OPEN | `47a1663f` | De TV-hero toont maximaal twaalf unieke geschikte recente films en een capsule voor bekeken, bezig of niet bekeken. De `/v1`-fixture mist releasedata; DEC-097 filtert die films terecht uit. Twaalf slides zijn via widgettests bewezen; een echte Plex/Jellyfin-compositorrun blijft open. Zie NLX2 en `docs/superpowers/plans/2026-09-23-netflix-redesign-tranche-1.md`. |
| UX23-PREF | Gedeelde instellingen-subpagina’s wijken visueel af van TV-overzichten. | UITERLIJK/HOME CODE+SIM DONE · FORMULIEREN OPEN · HARDWARE OPEN | `7e3af503` | Het oorspronkelijke Uiterlijk-beeld en de themapicker-aanname waren auditmateriaal, geen bewezen Back-bug. NLX2 levert TV-categorieën, actuele waarden en de nieuwe Home-indeling met compositorbewijs; NLX3 levert de zes-route-verbindingpicker. De onderliggende verbindingsformulieren en resterende vensters blijven open. |
| UX23-VERIFY | Oude Mijn Pleya-styling-audit verwacht Bibliotheken na twee keer DOWN; actuele hub staat daar op Instellingen. | OPEN, scenario-onderhoud | n.v.t. | Eerste run faalt met beelden op de verkeerde verwachte bestemming. Afzonderlijke actuele auditroute PASS in `tvos-ux23-current-surfaces-1790183928286`; bestaande scenario niet aangepast tijdens deze ontwerpaudit. |
| UX23-ROWS | Home-indeling toont twee rijen met dezelfde titel Recent toegevoegd en dezelfde servernaam Zolder; acties staan dicht bij de rechterrand. | CODE+SIM DONE · HARDWARE OPEN | `b2d70394` | Geen dubbele catalogusdata: de rijen heten nu `Films · Zolder` en `Series · Zolder`, met behoud van rij-ID’s, acties en Back-focus. Het gedeelde TV-paginakader en de safe insets zijn in `tvos.settings.home-layout` met compositorbeelden gecontroleerd. Zie NLX2. |
| HOME-REFRESH | Home toont geen nieuw toegevoegde films of series zolang de app open blijft (melding 24 sep 2026, vooral Apple TV). | CODE+SIM DONE · HARDWARE OPEN | `80d57172`, `4c3d7d12`, `9d7ac906`, `8e98f4a2`, `a85183af`, `65bfaf35`, `21e0ff80`, `46549639`, `1e24c73c`, `b9b9ac1e` | Oorzaak: `DiscoverProvider` laadde hubs alleen bij een volledige `load()`; terugkeer, resume en watch-events verversten alleen Verder kijken, en de lokale map hield zijn scan de hele sessie. Nu `refreshIfStale` (2 min bij terugkeer/resume, timer van 5 min zolang Home actief is), stil en zonder focusverlies. `tvos.home.recently-added-refresh` PASS in `tvos-home-recently-added-refresh-1790314296733` (fixronde 1: ook terugkeer uit een detail, review I2). Hardware: laat de Apple TV op Home staan, voeg een film toe en kijk of hij binnen vijf minuten verschijnt. |
| TS1 | Top Shelf: Play op de remote moet hervatten en een klik de detailpagina openen; daarna (besluit Michel) een carrousel in stijl `.actions`, eerst de hero, dan Verder kijken; op 25 sep (besluit Michel) stijl `.details` en Pleya Server-beelden via de app-groep; daarna (besluit Michel) titel en contextregel als eerste regel van de samenvatting. | CODE DONE · SIM GEZIEN · HARDWARE OPEN | `c307359c`, `dd103603`, `6680d2bf` | **Titelregel in de samenvatting:** omdat tvOS 26.5 `title` en `contextTitle` in `.details` niet tekent, zet de app ze als eerste regel in `summary` ("Wide 2.40 · Verder kijken · 42 min over"), daarna de echte samenvatting. Zonder samenvatting blijft alleen die regel over, en een samenvatting die al met de regel begint krijgt hem niet nog eens. Apple noemt geen lengtegrens voor `summary`; op de simulator kapt tvOS na vier regels af met een beletselteken en blijft de eerste regel staan. Simulator: de echte app-payload van het Pleya Server-item toont "Wide 2.40 · Verder kijken" naast Speel en Meer informatie; een nagebouwde payload met lange samenvatting toont de titelregel boven de afgekapte tekst. `title` en `contextTitle` gaan nog steeds mee. **Stand na `6680d2bf`:** de extensie levert `.details`. Na een klik op de schermvullende carrousel toont tvOS samenvatting, genre, jaar, duur en regisseur/cast (`namedAttributes`) naast Speel en Meer informatie. `title` en `contextTitle` gaan mee, maar de tvOS 26.5-simulator tekent ze in deze stijl niet, dus "Verder kijken · 42 min over" en de titel blijven onzichtbaar. De app downloadt elk carrouselbeeld via de artwork-cache (die de Pleya Server-header toevoegt) naar `TopShelfImages` in de app-groepcontainer en geeft een `file://`-URL door. Plex en Jellyfin gaan via hetzelfde pad, dus er staat geen token-URL meer in de carrousel. Simulator (signed build met app-groep, Verify-fixture Pleya Server): het backdrop van Wide 2.40 is zichtbaar op het Home-scherm en schermvullend, Speel start bij een koude start de speler, en een sync ruimt bestanden van verdwenen items op. Voor hardware nog te bekijken: beelden van Plex, Jellyfin en Pleya Server, en of het opruimen na een profielwissel werkt. Eerdere stand: Play stuurt `action=play`, klik `action=open`, links zonder action houden het oude afspeelgedrag (Android Watch Next). De extensie levert `TVTopShelfCarouselContent(style: .actions)`: eerst de films van de Home-hero (zelfde selectie en volgorde), dan Verder kijken; elk gebruikt veld is nagelopen in de TVServices-headers van de tvOS 26.5-SDK. De rij Nieuw toegevoegd uit `7a1374cd` is vervangen; de gesectioneerde rij Verder kijken blijft als terugval als geen item een beeld heeft of de payload van een oudere build komt. Een afgeleverde warme tik wist nu de geparkeerde koude-startlink. Op de simulator (eigen toestel, Verify-fixture) gezien: de carrousel verschijnt boven de app-rij, Speel en Meer informatie staan eronder, pijl rechts bladert naar het volgende item, Meer informatie opent bij een koude start de detailpagina en Speel start bij een warme start de speler. Twee beperkingen: `.actions` toont geen titel en geen contextregel, dus "Verder kijken · 42 min over" is onzichtbaar (alleen `.details` toont tekst); en Pleya Server-artwork vraagt een auth-header die de Top Shelf niet meestuurt, dus die backdrops blijven zwart (Plex en Jellyfin zetten hun token in de URL). De volgorde hero dan Verder kijken is met een nagebouwde payload bewezen, omdat de fixture geen releasedatum heeft en de hero daar leeg blijft. Beeld en knoppen op echte hardware nog bekijken. |
| SEARCH-FOCUS-BACK | tvOS: Menu vanaf een detailpagina die uit de zoekresultaten is geopend, zet de focus op het zoekveld in plaats van op de kaart waar de kijker vandaan kwam. | OPEN | n.v.t. | Al aanwezig op github/main `93ffb3da`, geen regressie van bundel 1 (review A, sectie 5). `_performSearch` in `lib/screens/search_screen.dart` parkeert de focus op `_searchFocusNode`, en er is geen focusherstel naar een kaart zoals `TvFocusRestoreHost` dat elders doet. `tvos.search.results.yaml` test de terugkeer van een detail niet. Volgende stap: focusherstel via de bestaande host plus een Verify-scenario met de terugweg. |
| SEARCH-PILL-UP | tvOS: UP vanaf de zoekpil doet niets. | OPEN | n.v.t. | Al aanwezig op github/main `93ffb3da`, geen regressie van bundel 1 (review A, sectie 5). De `FocusableButton` rond de pil in `lib/screens/search_screen.dart` heeft `onNavigateLeft`, `onNavigateDown` en `onBack`, maar geen `onNavigateUp`. Volgende stap: bepalen waar UP heen moet (de navigatiebalk) en dat met een Verify-assert vastleggen. |
| SHELF-TOKEN-URI | `posterUri` in Android Watch Next en in de tvOS-sectierij draagt nog een token-URL (`X-Plex-Token` of `api_key`). | OPEN, minor | n.v.t. | `_convertToShelfItem` in `lib/services/system_shelf_service.dart` zet `posterUri = client.thumbnailUrl(thumbPath)`. Op tvOS gaat dat naar de app-groep-`UserDefaults`, alleen leesbaar voor de app en de eigen extensie, dus klein risico; de carrousel draagt al alleen `file://`. Op Android leest de launcher, een ander proces, de URL met token. Bestond al vóór bundel 1 (review B, top-shelf M-1). Volgende stap: Android via een `content://`-provider serveren, op tvOS de sectierij ook via `file://` uit de app-groep. |
| SHEET-AUDIO-LABEL | iPhone-filtersheet: het Engelse railabel "Audio language" wordt afgekapt tot "Audio lang...". "Audiotaal" past. | OPEN, minor | n.v.t. | Gezien in `iphone_393x852_*.png` van de search-filters-fixronde (review A, m3). De rail is 112 pt breed en `_CategoryRow` in `lib/widgets/mobile/mobile_catalog_filters_sheet.dart` houdt het label op één regel. De rij bestond al vóór bundel 1. Volgende stap: twee regels toestaan of de rail verbreden, en dat met screenshots in de langste talen bewijzen. |
| VIS-0925-A | Hardwarefoto's build 303 (25 sep): in Light kleurt een gefocuste tegel donker (~#555) met donkere tekst erop. Oorzaak: `FocusTheme.focusDecoration` zet de J10-separator als `BoxShadow`, en een BoxShadow vult de hele box onder de 13%-vulling. De ringradius (6) past bovendien niet op pillen van 12. | FIXED, Verify-sim PASS, hardware open | `f76568bb`, `a625b28d`, `6dfbf602`, `fc12f06f` | Pixeltest `test/focus/focus_ring_fill_pixel_test.dart`: midden van een gefocuste tegel is in Light, Dark en OLED gelijk aan de vulling (rood op de oude code: 0x525252). Sim: `tvos.glass.topnav-light`, `tvos.my-pleya.*`. Goldens stale, regen via `goldens.yml`. Review-fixronde: bij een binnenring blijft de separator binnen de box, zodat een clippende kaart hem niet meer wegsnijdt (pixeltest `test/widgets/settings_row_separator_clip_test.dart`, eerste en laatste rij). Hercontrole: de separator gaat alleen binnen de box bij de instellingenrijen (`separatorInside`); elke andere ring houdt 2,5 px wit met de lijn erbuiten, test met opaak kind (`test/focus/focus_ring_opaque_child_test.dart`). |
| VIS-0925-B | Light: geselecteerd en gefocust zijn niet te onderscheiden; `surfaceElevated` #EDEDED, `surface` #FFF en `bg` #F7F7F8 scheiden nauwelijks. Dark/OLED: 1px-randen op kaarten en pillen. | FIXED, Verify-sim PASS, hardware open | `a4d911af` | Contrastratio's per palet in `test/widgets/tv_appearance_categories_test.dart`. Sim: `tvos.settings.appearance-categories`, `tvos.glass.topnav-light` (01b). |
| VIS-0925-C | Instellingenkaart oogt als een tabel (buitenrand plus scheidingslijnen), `_languageSelector` mist `SettingRowFocus`, density-labels in hardcoded grijs, Switch-track verdwijnt in Light, lege `_ProfileHeader`-balk op Mijn Pleya. | FIXED, Verify-sim PASS, hardware open | `d135b813`, `6f744136`, `ef9c3de6`, `7550dca4` | Rijfocus in `test/screens/settings/settings_row_focus_test.dart`, Switch in `test/theme/switch_visibility_test.dart`. Sim: `tvos.settings.appearance`, `tvos.glass.topnav-light`, `tvos.my-pleya.sections`. Review-fixronde: het switchthema geldt alleen op de TV-instellingenrijen (via `TvSettingsDensity`); selected en disabled houden de Material-standaard. Hercontrole: het TV-switchthema (`TvSwitchTheme`) geldt ook in de overlaysheets op TV. |
| VIS-0925-D | Glastokens negeren het thema: in Light een platte grijze topbalk, in OLED alleen een harde witte rand. De rim van 1,5 px schaalt niet mee. | FIXED, Verify-sim PASS, hardware open | `562a7ec2` | `test/theme/glass/tv_glass_tokens_theme_test.dart`, Light-test in `tv_top_navigation_glass_test.dart`. Sim: `tvos.glass.topnav` en `tvos.glass.topnav-light`. Tier blijft fake (DEC-122). |
| VIS-0925-E | Hero in Light: de scrim is een witte waas over 60% van het beeld; CTA-ring in `tk.text` op 4 pt afstand geeft een zwart-witte dubbele rand; titel, CTA en eerste railtegel staan op drie x-posities. | FIXED, Verify-sim PASS, hardware open | `22e94b0e`, `b7a743e2`, `a5bb92f7`, `d23d5fa3`, `2d73dec1` | Synopsis 4,5:1 over de hele kolom in `test/widgets/tv/tv_hero_scrim_light_test.dart`, x-lijn in `tv_content_feed_test.dart`. De eerste versie zette de hero op 83,34; na de review weer op 75,48 (hieronder). Review-fixronde (besluit Michel D1, D3): hero-titel, CTA-vulling en railkop terug op de paginarand (75,48), alleen de tegelartwork springt een ringband in; CTA-ring met dezelfde vrije band als de categorierail. Light-hero even helder als Dark: de +0,08 is overal weg (ook gedimd), de Light-leesscrim volgt buiten de tekstkolom de donkere ramp (pixeltest rechter 40%). Scenario's `tvos.home.hero-cta-focus`, `tvos.theme.dark-hero-cta`. Hercontrole: Light-leesscrim vlak 0,54 over de tekstkolom, daarna met de steilste helling van Dark omlaag, vanaf 73,6% gelijk aan Dark; geen naad meer (pixeltest 28% tot rand, Light hoogstens Dark plus 0,303). |
| VIS-0925-F | OLED: grijze tekst op de witte actieve topnav-pil en halo's rond instellingentekst. De code geeft #000 op de pil en zet geen glow of text shadow op instellingen; waarschijnlijk camera-bloom. | HARDWARE ONLY | n.v.t. | Geen fix. Nakijken in Michels hardwareronde met dezelfde OLED-foto's. |
| VIS-0925-G | Density: Mijn Pleya-tegels van 151 pt met 19 pt ondertitel, cataloguskaarttekst onder 23 pt, CTA-label 25 pt, veel lucht in Instellingen, `NoticeHost` buiten `_AppleTvScale`. | FIXED, Verify-sim PASS, hardware open | `fadb39e5`, `2e7228c7` | Zie DEC-139. `test/widgets/tv/tv_dens1_hig_test.dart` (rijen 56/84 pt, kaarttekst >= 23 pt), hub past op 1038x584 (`tv_my_pleya_screen_test.dart`). Voor/na-schermen uit Verify bij de review. Review-fixronde: hub-ondertitels over twee regels, test met Inter-metrics (en, nl) dat niets wordt afgekapt. |
| REQ-SEARCH-ROUTE | Aanvragen (tvOS): DOWN vanaf het zoekveld doet niets, UP loopt dood, UP uit de content slaat het veld over, Menu op een leeg veld springt naar de topnav. Oorzaak: `seerr_discover_screen.dart` geeft het TV-veld desktop-callbacks naar focusnodes die op TV nooit gebouwd worden. | FIXED, Verify-sim PASS, hardware open (Siri Remote) | `102fd9b4`, `6f744136`, `494f5703` | Widgettest met echt focusbaar veld in `test/screens/tv/tv_seerr_discover_view_test.dart` (rood met de oude onExitTop), Verify `tvos.requests.search-route` PASS. Review-fixronde: gefocust veld krijgt de witte ring, veld start op de titel-x (besluit Michel D2). |
| DPR1 | Apple TV 4K geeft 4K uit, maar de UI oogt als opgeschaalde 1080p. Vraag was of de DPR die de engine-fork uit `windowScene.screen.scale` haalt 2,0 is. | VERIFIED (hardware, geen codewijziging) | n.v.t. | Gemeten in log `ekeb2` (build 305): UIScreen 1920x1080, nativeBounds 3840x2160, scale 2,0; FlutterView 3840x2160 bij dpr 2,0; binnen `_AppleTvScale` 1037,8x583,8 bij dpr 3,7. De geplande logregel vervalt (besluit Michel); alleen de onjuiste doc-comment bij `_AppleTvScale` is gecorrigeerd. Uitwerking in DEC-139. |
| VIS-0925-H | Light-contrastaudit (review, Michel): geen lichte tekst op licht. | FIXED, Verify-sim PASS, hardware open | `7fa614c7`, `64010d9f`, `140347fa` | `test/theme/tv_text_contrast_audit_test.dart` meet elke TV-tekst- en icoontoken tegen zijn echte achtergronden in Light, Dark en OLED (WCAG AA per effectieve puntmaat). Tertiaire inkten 0,5/0,56/0,42 naar 0,62/0,6 en de gedempte selectietag naar de tertiaire inkt; Dark en OLED alleen sterker. Visuele sweep: `tvos.light.contrast-sweep`. Hercontrole: catalogus-secundair 0,62 naar 0,7 zodat de stap naar tertiair terug is (test: stap minstens 0,06 in elke ladder); de globale waarde in SeriesLanguagePanel houdt buiten TV 0,5. |
| VIS-0926-S1 | Hardwarefoto build 304 (Uiterlijk > Startscherm, Dark): na VIS-0925-G staat alles dicht op elkaar. Titel en waarderegel raken elkaar bijna, scheidingslijnen liggen tegen de tekst, de kaart stopt op driekwart van het scherm en de categorierail oogt als een stapel. | FIXED, Verify-sim PASS, hardware open | `1d0c403e` | Zie het amendement onder DEC-139. Rijen 70/106 pt (G: 56/84, voor G: 60/92), titelregel 38 pt, waarderegel 36 pt, inzet 16 pt; sectielabel 28/12 pt; categoriepil 92 pt met 16 pt ertussen (was 80 en 8). Tekst blijft 29/25 pt. `test/widgets/tv/tv_dens1_hig_test.dart` pint rijen, leading en rail in punten, en eist dat zes rijen Startscherm 90% van hun kolom vullen of scrollen (met de G-waarden 80%, rood). Verify-geometrie: laatste rij eindigt op 1000 pt, was 836, veilige rand 1020. Scenario's `tvos.settings.appearance`, `tvos.settings.appearance-categories`, `tvos.my-pleya.section-settings` en `tvos.light.contrast-sweep` PASS. Geen golden rendert instellingenrijen. Hardware: zelfde scherm op de 77 inch tv fotograferen. |
| LG-SKIP1 | Vraag Michel: waarom is de knop Intro/Aftiteling overslaan niet van glas? DEC-122 noemt knoppen en spelerbediening, maar `SkipMarkerButton` hing nooit aan het glaspad en bleef de witte pil. | FIXED, widgettest PASS, sim en hardware open | zie commit | Met glas aan (iPhone en Apple TV, dezelfde poort als de rest van de speler) een `GlassSurface` zonder backdrop: Apple TV `GlassTokens.tvFor(panel: true)` (zwart 70%), iPhone de spelerplaat met zwart 60% in plaats van 55%. Witte tekst en icoon met de glasschaduw, maat en positie gelijk, focusring en D-pad ongewijzigd; glas uit blijft de witte pil. Contrast over wit beeld: vlak (tint over wit) iPhone 5,74 (55% zou 4,74 zijn), Apple TV 8,59, iPhone met rode aftelvulling 6,26; p95 in pixels iPhone 5,33 en 5,94 met aftellen, Apple TV 7,81 en 7,96. Met 55% haalde de pixelmeting 4,48 door de lichtrand bovenin. Test: `test/widgets/video_controls/skip_marker_glass_test.dart`. De Verify-fixture levert geen markers, dus het bewijs is een widgetrender over wit, Bunny en donker. Geen golden rendert de knop. iPad, Android en desktop veranderen niet. |

...
| VIS-0925-A | Hardwarefoto's build 303 (25 sep): in Light kleurt een gefocuste tegel donker (~#555) met donkere tekst erop. Oorzaak: `FocusTheme.focusDecoration` zet de J10-separator als `BoxShadow`, en een BoxShadow vult de hele box onder de 13%-vulling. De ringradius (6) past bovendien niet op pillen van 12. | IN PROGRESS | n.v.t. | Separator als tweede lijn buiten de witte ring, geen schaduw. Regressietest: pixel midden in een gefocuste tegel in Light is gelijk aan de 13%-vulling. Branch `fix/tvos-visual-hardware-0925`. |
| VIS-0925-B | Light: geselecteerd en gefocust zijn niet te onderscheiden; `surfaceElevated` #EDEDED, `surface` #FFF en `bg` #F7F7F8 scheiden nauwelijks. Dark/OLED: 1px-randen op kaarten en pillen. | IN PROGRESS | n.v.t. | Geselecteerd is een gevulde pil zonder ring, gefocust is ring plus schaal. Contrastratio's vastgelegd in een test. |
| VIS-0925-C | Instellingenkaart oogt als een tabel (buitenrand plus scheidingslijnen), `_languageSelector` mist `SettingRowFocus`, density-labels in hardcoded grijs, Switch-track verdwijnt in Light, lege `_ProfileHeader`-balk op Mijn Pleya. | IN PROGRESS | n.v.t. | Eén focustaal per pagina. |
| VIS-0925-D | Glastokens negeren het thema: in Light een platte grijze topbalk, in OLED alleen een harde witte rand. De rim van 1,5 px schaalt niet mee. | IN PROGRESS | n.v.t. | Themabewuste `GlassTokens.tv`, tier blijft `fake` (DEC-122). Hardware: OLED-topnav-tekst nakijken. |
| VIS-0925-E | Hero in Light: de scrim is een witte waas over 60% van het beeld; CTA-ring in `tk.text` op 4 pt afstand geeft een zwart-witte dubbele rand; titel, CTA en eerste railtegel staan op drie x-posities. | IN PROGRESS | n.v.t. | Kleinere scrim, één CTA-focuscontract, x-uitlijning in een test. |
...
| VIS-0925-G | Density: Mijn Pleya-tegels van 151 pt met 19 pt ondertitel, cataloguskaarttekst onder 23 pt, CTA-label 25 pt, veel lucht in Instellingen, `NoticeHost` buiten `_AppleTvScale`. | IN PROGRESS | n.v.t. | Gericht verdichten via `TvHig`, zie DEC-139. DEC-028 blijft op 1,85. |
| REQ-SEARCH-ROUTE | Aanvragen (tvOS): DOWN vanaf het zoekveld doet niets, UP loopt dood, UP uit de content slaat het veld over, Menu op een leeg veld springt naar de topnav. Oorzaak: `seerr_discover_screen.dart` geeft het TV-veld desktop-callbacks naar focusnodes die op TV nooit gebouwd worden. | IN PROGRESS | n.v.t. | Route topnav, veld, eerste kaart en terug; widgettest `test/screens/tv/tv_seerr_discover_view_test.dart` en Verify-scenario `tvos.requests.search-route`. |

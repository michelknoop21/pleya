...
| DENS1 | F-TV1, dichtheidsaudit (`docs/density-audit-2026-09.md`): de gedeelde `showOptionPickerDialog` (nu `lib/utils/option_picker_dialog.dart`, tot `43083a1b` in `dialogs.dart`; gebruikt door o.a. `media_context_menu.dart`, `record_options_sheet.dart`, `live_tv.dart`) sizede paneel en rijen met rauwe literals die op TV alleen de globale 1,85x-wrapper zagen, nooit `TvLayoutConstants.scaleOf`, zo'n 18% ruimer dan een getokend paneel bij dezelfde nominale waarde. Elke literal gaat nu door `scaleOf` op TV (`scale` is 1,0 elders, dus geen wijziging buiten TV). Nagekomen (review, 15 sep, finding 5): de eerste ronde miste `contentPadding` en beide `AppIcon`-iconen, dus "elke literal" klopte nog niet; nu ook geschaald | FIXED | `792da906`, `1bb5ae45` |

...
| NLX2 | Goedgekeurd Netflix-redesign vervolg: Uiterlijk en Home-indeling op actuele simulatorbeelden beoordelen en TV-presentatie verbeteren. Behoud iedere bestaande voorkeursleutel, platformvoorwaarde, rij-ID, volgorde-/zichtbaarheidsactie en Back-focus. De twee identieke `Recent toegevoegd / Zolder`-rijen tonen nu `Films · Zolder` en `Series · Zolder`; Home-indeling gebruikt het gedeelde TV-paginakader en blijft na Menu op de oorspronkelijke tegel. Uiterlijk en de verdere functiepariteit zijn nog open. | IN PROGRESS · HOME SIM VERIFIED · UITERLIJK OPEN · HARDWARE OPEN | branch `codex/netflix-redesign-tranche-2`; `tvos.settings.home-layout` PASS met compositorbeelden in `.build/pleya-verify/tvos-settings-home-layout-1790193631774/`; gerichte widget-/providertests en `scripts/ci_checks.sh` groen; plan `docs/superpowers/plans/2026-09-23-netflix-redesign-tranche-2.md` |

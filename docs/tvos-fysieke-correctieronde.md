...
| PLR8 | Melding van Michel (19 september): op zowel tvOS als iOS lijkt het beeld merkbaar donkerder te worden zodra de zoomlevel in de speler wordt gewijzigd, alsof er iets misgaat in de HDR-verwerking. Geen reproductiepoging, geen root cause, geen hardware-sessie: puur de melding, vastgelegd zodat hij niet verloren gaat. Cross-platform (beide OS'en genoemd), dus vermoedelijk een gedeeld mpv/HDR-pad, niet een los TV- of iOS-scherm. Player-paneel-zoomcode zit in dezelfde familie als PLR2/PLR7 (`tv_panel_widgets.dart`, `kTvPanelZoomPresets`), maar de iOS-kant van dezelfde zoombediening is niet onderzocht | OPEN, niet gereproduceerd | n.v.t. | Eerst reproduceren (simulator zonder HDR-scherm bewijst hier weinig, dus waarschijnlijk hardware-only) en vaststellen of dit een beeldverhouding/zoom-crop-effect is dat op een HDR-paneel feller opvalt, of een echte metadata-/tone-mapping-regressie bij een boxFit-wissel |

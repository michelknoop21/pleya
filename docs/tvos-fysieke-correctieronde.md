...
| SYS-1d | Twee TV-contentroutes pushen nog met een kaal `Navigator.of(context).push(MaterialPageRoute(...))` in plaats van via `openTvContentRoute`/`tvContentRouteRegistry`, en dekken daarmee de shell af zoals SYS-1a het contract precies verbiedt. Eerste plek: `TvLibrariesScreen._openInCatalog` in `lib/screens/tv/sections/tv_libraries_screen.dart:220`, de "Openen in catalogus"-actie op een bibliotheek in Instellingen. Tweede plek: `TvContentFeedState._openViewAll` in `lib/widgets/tv/tv_content_feed.dart:369`, de ROW1c "Alle N"-tegel op een viewer-eigen Home-rij. Beide openen `TvUnifiedCatalogScreen`, hetzelfde scherm dat `main_screen.dart`'s `_openTvCompleteCatalog` wél correct via `_pushTvContentRoute`/`tvContentRouteRegistry` opent, dus het contract bestaat al voor dit scherm en deze twee aanroepen volgen het niet. Reproductie: op de Apple TV naar Instellingen > Bibliotheken, een filmbibliotheek selecteren, "Openen in catalogus" kiezen: de bovennavigatie verdwijnt; of op Home een viewer-eigen rij openen en op de "Alle N"-tegel drukken, zelfde effect | OPEN | n.v.t. |
...
| TOK2 | Serverstip `#3FBF5F` hardcoded in `tv_my_pleya_screen.dart:833` | OPEN | n.v.t. |

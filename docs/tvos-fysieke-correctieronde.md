...
| OFF6 | Een rebind na herverbinden wist de geopende Mijn Pleya-sectie: `binder.rebindActive()` → `invalidateNow` → `_invalidateAllScreens` → `_tvNav.clearNestedRoutes()`, en de kijker landt op de kale hub | OPEN, besluit 24 sep: sectie behouden (TV9) | n.v.t. | Gevonden door de OFF5-test (TV6, 23 september). Treedt op wanneer `MainScreen` online komt zonder zichtbaar verbonden server, of na Opnieuw verbinden wanneer `shouldRetryActiveProfileBindAfterReconnect` waar is. Open vraag: moet een rebind voor hetzelfde profiel de geneste routes wissen? Niet gerepareerd in TV6. |
| PROF1 | `nav.profile` opent op TV de generieke `ProfileSwitchScreen`-lijst (volle breedte, geen TV-inzet, mobiele stijl) en niet de poortcompositie van mockup 21; `TvProfileGate` verschijnt alleen met `requireSelection: true` | OPEN, besluit 24 sep: `TvProfileGate` (TV9) | n.v.t. | Gevonden in de TV7-run van `tvos.profile.picker` (23 september). Open vraag: moet de kiezer vanuit de balk ook de TV-poort tonen? Niet gerepareerd in TV7. |

...
| VIS-0925-A | Hardwarefoto's build 303 (25 sep): in Light kleurt een gefocuste tegel donker (~#555) met donkere tekst erop. Oorzaak: `FocusTheme.focusDecoration` zet de J10-separator als `BoxShadow`, en een BoxShadow vult de hele box onder de 13%-vulling. De ringradius (6) past bovendien niet op pillen van 12. | FIXED, Verify-sim PASS, hardware open | `f76568bb`, `a625b28d` | Pixeltest `test/focus/focus_ring_fill_pixel_test.dart`: midden van een gefocuste tegel is in Light, Dark en OLED gelijk aan de vulling (rood op de oude code: 0x525252). Sim: `tvos.glass.topnav-light`, `tvos.my-pleya.*`. Goldens stale, regen via `goldens.yml`. |
...
| VIS-0925-E | Hero in Light: de scrim is een witte waas over 60% van het beeld; CTA-ring in `tk.text` op 4 pt afstand geeft een zwart-witte dubbele rand; titel, CTA en eerste railtegel staan op drie x-posities. | FIXED, Verify-sim PASS, hardware open | `22e94b0e`, `b7a743e2` | Synopsis 4,5:1 over de hele kolom in `test/widgets/tv/tv_hero_scrim_light_test.dart`, x-lijn in `tv_content_feed_test.dart`. Hero-knop op 83,34 in `tvos.my-pleya.alignment`: de tekst schuift naar de artworkrand van de railtegel (VER3), in plaats van de tegel uit de veilige zone. |

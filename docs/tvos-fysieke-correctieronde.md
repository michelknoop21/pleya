...
| TOK2 | Serverstip `#3FBF5F` hardcoded in `tv_my_pleya_screen.dart:830` | OPEN | n.v.t. |
...
| CTA1 | `test/theme/no_local_cta_shape_override_test.dart` faalt op main: `lib/screens/media_detail/mobile_detail_view.dart:265` en `:307` zetten lokaal `shape: const StadiumBorder()` op `FilledButton.styleFrom`, buiten `MonoShapes.cta` om | OPEN | n.v.t. | Gevonden tijdens de OFF3-triage van de bredere Unit Tests-CI-faling (14 september), niet door OFF3 of OFF4 veroorzaakt: geen van beide raakt `mobile_detail_view.dart`. De test bestaat specifiek omdat een losse `shape:`-override eerder de focusring in vier andere schermen buiten een pilvormige knop liet vallen (zie de test-eigen docstring). Hier lijkt de pilvorm bewust (een witte "Afspelen"-knop en een "Downloaden"-knop), dus vermoedelijk hoort dit bestand bij de `allowed`-lijst in de test met een reden, niet een verwijderde `shape:`. Niet zelf besloten zonder opdracht: dit is een mobiel scherm, buiten de scope van deze tvOS-ronde |
| HTTP1 | `test/utils/http_lifecycle_test.dart` faalt wisselend op main: "ManagedHttpClient closeGracefully can retry after a drain timeout" verwacht `drainTimedOut: false`, krijgt `true` | OPEN | n.v.t. | Gevonden tijdens dezelfde OFF3-triage. Geen golden, geen pixelvergelijking: een tijdgevoelige test die op de zwaarder belaste Linux-CI-runner een kortere drain-deadline kennelijk wel laat verlopen. Niet gereproduceerd op de gepinde 3.44.0-SDK lokaal. Niet zelf opgepakt zonder opdracht |
| HERO6 | Terug op Home vanuit een Verder kijken-item staat de ring op de topnav, de feed gescrold, en Hervatten en Details bekijken staan dwars door de navigatiebalk (foto 14 september). Voorrang boven OFF2 t/m OFF4 op verzoek van Michel. Verificatie alleen in de simulator, geen device-run. Gereproduceerd in de tvOS-simulator met alleen twee keer omlaag en Menu vanaf de rij: de ring op Home, de feed op 721 px, de CTA-rij op y=19 zichtbaar. Oorzaak in `TvContentFeed.build`: `textOpacity` hing alleen aan `_rowHasFocus`, dus zodra de focus de feed verliet zonder te scrollen kwamen titel en CTA's terug op de verschoven plek. Fix: ook verbergen zolang de feed niet bovenaan staat (`_rowHasFocus \|\| !_atTop`); `_revealHero` zet `_atTop` terug zodra een CTA de ring krijgt. Negatieve controle `HERO6: a scrolled feed keeps the hero text hidden when the focus leaves the rows` in `tv_content_feed_test.dart`: rood op `textOpacity == 0` tegen de oude code, groen erna, 70 tests groen. Nieuw Verify-scenario `tvos.home.hero-cta-hidden-scrolled` (via `discover.hero` `state.textVisible`) PASS in bundel `-1789383221540`; `tvos.home.hero-return` PASS. `tvos.home.hero-return-from-route` faalt, maar identiek zonder deze wijziging: zie HERO7. De 11 falende `tv_home_production` goldens falen lokaal met en zonder de wijziging met dezelfde pixelaantallen | VERIFIED | `5bac502e` |
...
| FOC1 | Als de gefocuste TV-topnav-pil verdwijnt (Live TV die wegvalt, of sinds `472233db` elke offline-flip), zet `TvNavigationCoordinator` de logische `_focused`/`_active` wel om, maar niets roept `FocusNode.requestFocus()` aan op de vervangende pil. `_focus`/`_focusKey` in `tv_top_navigation.dart` lopen alleen op een expliciete D-pad-stap (`onNavigateLeft`/`onNavigateRight`), niet op een balkherberekening. Gevonden door een Codex-challenge op `472233db`; bestond al voor de Live TV-casus, offline maakt hem alleen vaker bereikbaar. Geen widgettest kan dit vandaag bewijzen (geen enkel testbestand construeert `MainScreen`/`TvRootShell` samen) | OPEN | n.v.t. |
...
| SEARCH2 | Op Zoeken op TV bleven bij verticaal navigeren resultaatkaarten zichtbaar door de vaste zoekheader/topnav; op de fysieke Apple TV 4K stond de header bovendien niet volgens het TV-layoutcontract en vervormde de losse resultaattelling de zoekpill (Michel, simulator-screenshot 13 september en fysieke foto `IMG_8473.jpeg`, 15 september). Root cause: de vaste header stond correct buiten de scroll, maar de verticale `SingleChildScrollView` in `TvSearchView` gebruikte `Clip.none` om horizontale focusringen ruimte te geven en liet daardoor óók verticaal buiten zijn viewport schilderen; `_buildTvSearchField` gebruikte daarnaast lokale 16/12-padding en bouwde het totaal als `InputDecorator.suffixIcon`. Gefixt bij de gedeelde eigenaar: een tokenized, opaque `search.header`, een afzonderlijk count naast de stabiele pill, een geclipte verticale viewport die de horizontale rail-headroom intact laat, en secties die vóór de focuswissel op hun verticale anker scrollen zonder een tweede generieke verticale kaartscroll. Negatieve controles bewaken de header/bodygrens, onafhankelijke countgeometrie en volledig zichtbare focuskaart. `tvos.search.header-scroll` bewijst top, eerste kaart, RIGHT, één/meerdere banden omlaag zonder afgesneden vorige band, terug naar de header en weinig/veel resultaten; simulatorbundel `.build/pleya-verify/tvos-search-header-scroll-1789501402368` is groen. Hardwarevergelijking blijft als eindcontrole open | FIXED, simulator geverifieerd, hardwarevergelijking open | (deze commit) |
...
| DENS1 | F-TV1, dichtheidsaudit (`docs/density-audit-2026-09.md`): de gedeelde `showOptionPickerDialog` (nu `lib/utils/option_picker_dialog.dart`, tot `13c53c69` in `dialogs.dart`; gebruikt door o.a. `media_context_menu.dart`, `record_options_sheet.dart`, `live_tv.dart`) sizede paneel en rijen met rauwe literals die op TV alleen de globale 1,85x-wrapper zagen, nooit `TvLayoutConstants.scaleOf`, zo'n 18% ruimer dan een getokend paneel bij dezelfde nominale waarde. Elke literal gaat nu door `scaleOf` op TV (`scale` is 1,0 elders, dus geen wijziging buiten TV). Nagekomen (review, 15 sep, finding 5): de eerste ronde miste `contentPadding` en beide `AppIcon`-iconen, dus "elke literal" klopte nog niet; nu ook geschaald | FIXED | `792da906`, `d30644c7` |
| DENS2 | F-TV2, dichtheidsaudit: getoetst of TV-detail nog een B-afwijking tegen mockup 37 draagt (`docs/tvos-redesign-37-approved.md`, DEC-109). Geen gevonden: DET2/DET3/DET6/DET7 hierboven hebben de compositie al op het mockup-37-contract gebracht (drie regels synopsis, `bottomSafeInset`, volledig scherm), met eigen negatieve controles in `media_detail_screen_test.dart`. F-TV2 is dus niet uitgevoerd deze ronde: er is niets om te fixen | GEEN AFWIJKING, F-TV2 niet getriggerd | n.v.t. |
| DENS3 | TV Home-density (rapportpunt, buiten de fixronde op Michels expliciete instructie): hero, rails, focus- en interactie-footprints en de viewportzones zijn gemeten en gerangschikt in `docs/density-audit-2026-09.md` §1d/1e, maar er is bewust niets aan de Home-hero gewijzigd. Vervolgwerk (indien gewenst) staat daar als gerangschikte ingreep, niet hier als losse regel | OPEN, rapportpunt | n.v.t. |
| DENS4 | Kijkafstandsoordeel 2160p: of de gemeten dichtheid op een echte 4K-tv, op normale kijkafstand, ook echt als "opgeblazen" aanvoelt is niet op een fysiek toestel getoetst: de audit toont dat 1080p en 4K identieke screen-space-layout geven (`docs/density-audit-2026-09.md`, "2160p is niet de oorzaak"), maar het perceptuele oordeel zelf blijft simulator-ontoegankelijk | HARDWARE ONLY | n.v.t. |
| DENS5 | Procesafwijking, review 15 sep (finding 6): DENS1 t/m DENS4 zijn pas toegevoegd in commit `8b13e923`, ná de drie fix-commits die ze beschrijven (`a2587806`, `05edee22`, `792da906`), terwijl de regel is de rij vooraf te registreren. Geen geschiedenis herschreven om dit te verbloemen; functioneel geen impact, alles landde vóór push op dezelfde branch. Vanaf de volgende correctieronde weer vooraf registreren | VASTGELEGD, geen functionele impact | n.v.t. |

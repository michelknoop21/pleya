...
| DET5 | Filmdetail op een echte Apple TV gebruikt het venster niet volledig: er staat een horizontale lijn op ongeveer 80% van de schermhoogte, waaronder niets rendert (Michel, twee foto's op hardware, 13 september; welke build/SHA op het toestel staat is nog niet bevestigd). Bevestigd Pleya-specifiek: het tvOS-beginscherm en Netflix op hetzelfde toestel gebruiken het venster wel volledig, dus geen tv-/tvOS-overscaninstelling. Het symptoom komt overeen met OVR1a (`NOT REPRODUCED`: `scaleForHeight`'s ondergrens 0,85 maakt TV-paneelinhoud ~1,5x te groot) en met DET1, die als "FIXED, hardware open" stond maar dus nog nooit op een fysiek toestel is bevestigd; DET3 (13 september, `ddbd2ce3`) wijzigde dezelfde surface (`spotlightTop`/`MediaQuery.padding.top` in `media_detail_screen.dart`, de topband-positionering in `tv_root_shell.dart`) en is zelf ook nog niet op hardware getest. Root cause nog te bepalen | OPEN | n.v.t. |

...
| REQ1 | Aanvragen end-to-end in Pleya Verify | GEBLOKKEERD op het fixture, niet op de code | n.v.t. | Alle aanvragen en Ontdekken zijn gebouwd (CAT11, mockup 35) en dragen sindsdien `tv.catalog.grid[requests]`, `tv.catalog.grid.item[requests.N]`, `tv.catalog.rail[requests]`, `tv.catalog.rail.row[requests.status]`, `tv.catalog.state[requests.*]` en dezelfde familie onder `discover`, dus beide schermen zijn meetbaar. Wat ontbreekt is dezelfde soort fixture als bij WL2: `pleya_verify/fixture_server` kent geen Seerr, spreekt alleen `/v1/*`, en zonder Seerr-sessie tekent Mijn Pleya de tegel Aanvragen niet. Een scenario zou hier niet falen maar nooit kunnen slagen. Sluit hem zodra het fixture een Seerr-eindpunt voert, of leg vast dat de dekking op een echte Jellyseerr-aanmelding hoort |
...
| RAIL2 | Op Home lijkt de focus in een rail bij herhaald RIGHT, en volgens Michels aanvulling ook bij LEFT, af en toe een positie over te slaan en meteen een aantal tegels verder te landen (Michel, hardware, build 272, log `h6ocl`: "Ik heb ook af en toe bij home in de rails met naar rechts klikken meerdere malen dat hij soms een positie lijkt over te slaan dus gelijk naar een aantal items verder schiet. Ook bij left lijkt het te gebeuren"). Build 272 is ouder dan de branch waar CAT18/CAT19 op zitten: apart focus-/traversalprobleem, geen uitloper van die twee. Log `h6ocl` opgehaald en gelezen: alle pijltjestoetsen op Home komen binnen als `deviceType=KeyEventDeviceType.keyboard`, via `AppleTvRemoteTouchService`'s vertaling van de Siri Remote-swipe naar synthetische pijltjestoetsen, niet als `directionalPad`. Eigenaar gevonden: binnen een rail-rij houdt `TvDiscoveryRail` geen eigen index bij, `onNavigateLeft`/`onNavigateRight` zijn alleen op de randen gebonden (`tv_discovery_rail.dart:667-668`, index 0 en `_lastIndex`); alle tussenliggende stappen lopen via Flutter's eigen richtingsgebonden focus-traversal. `_revealFocused` (regels 419-451) schuift de band met `_scroll.animateTo` over `FocusTheme.getAnimationDuration` (standaard 150 ms, `focus_theme.dart:49`), en de bestaande docstring zelf legt uit dat die traversal de tegel meet op het moment de focus landt, wat tijdens een nog lopende scroll-/expand-animatie niet de rustende rasterpositie is. Hypothese: het log toont series snelle losse drukken met tussenpozen van 164 tot 435 ms tussen twee "handled"-navigaties, een deel daarvan onder of net boven die 150 ms; komt een volgende druk aan terwijl de vorige animatie nog loopt, dan meet Flutter's traversal de tegels in een tussentijdse geometrie en kan de "dichtstbijzijnde kandidaat"-heuristiek een tegel verder dan de directe buur kiezen. Dat verklaart waarom het intermitterend is, bij zowel LEFT als RIGHT optreedt (generiek aan de traversal, niet aan een specifieke handler) en alleen bij snel herhaald klikken. Niet bewezen: het log toont alleen dat drukken snel genoeg zijn om te overlappen, niet welke tegel de focus kreeg. Reproductiepoging in de tvOS-simulator gestaakt: alle drie de profielen op dit simulator-toestel zitten achter een PIN of een wisselflow die niet doorliep zonder die PIN. Volgende stap: reproduceren met een profiel zonder PIN-lock, of op hardware | OPEN, hypothese vastgelegd | n.v.t. |

...
| OFF2 | Offline-tab-routingbug: `_handleOfflineStatusChanged` overschrijft de zojuist gekozen offline-tab | CONFIRMED ONBEREIKBAAR OP ECHTE APPLE TV | n.v.t. | Nameting wijkt op twee punten af van de oorspronkelijke `/code-review`-hypothese uit de OFF-1-ronde. Ten eerste de root cause: niet de `displaced`-tak van `_syncTvDestinations()` (die raakt op TV nooit een Mijn Pleya-sectie), maar `_selectTab`'s `isUserInitiated`/`previousTab != tab`-guard zelf, die nooit vuurt tussen Bibliotheken/Kijklijst/Aanvragen/Downloads/Instellingen omdat die op TV allemaal onder de ene root-tab `NavigationTabId.myPleya` leven (hoofdstuk 18.2). Ten tweede, en dat is de reden voor deze status: de latch die deze bug veroorzaakt (`_autoSwitchedToDownloads`) kan op een fysieke Apple TV nooit op `true` komen te staan. `NavigationTab.getVisibleTabs` (`navigation_tabs.dart:254`) sluit `NavigationTabId.downloads` onvoorwaardelijk uit zodra `PlatformDetector.isAppleTV()` waar is, ook offline; die regel dateert van 2 juli, ruim vóór deze bevinding. Beide plekken die de latch zetten (`main_screen.dart:738` bij opstarten, `:1707` in `_handleOfflineStatusChanged`) doen dat alleen als `_normalizeTabForMode`/`getVisibleTabs` `.downloads` teruggeeft, en dat pad loopt onvermijdelijk door diezelfde uitsluiting. `_lastOnlineTabId`/`_autoSwitchedToDownloads` is bovendien de enige "onthoud en herstel"-latch in het bestand: geen enkele andere tab heeft een vergelijkbaar mechanisme, dus er schuilt geen Apple-TV-bereikbare variant onder een andere naam. De bug is wél echt en bereikbaar, maar dan op Android TV: `platform_detector.dart` detecteert die apart via `Platform.isAndroid`/`AndroidTvFeatureDetection` met `isTV()==true` én `isAppleTV()==false`, en op dat platform blijft Downloads gewoon zichtbaar. Fix `0bda71a9` (`shouldCancelAutoSwitchedToDownloads` in `_openTvMyPleyaSection`) blijft staan als correcte, geteste hardening voor Android TV en de debug-forced-TV-modus, maar sluit deze rij niet: die is specifiek voor fysieke Apple TV-bevindingen. PR #27 is dienovereenkomstig herkaderd |
| OFF3 | `test/goldens/tv_discovery_landing_production_golden_test.dart` faalt op main: `_FakeClient` mist `fetchRecentlyAdded`/`fetchRecentlyAddedShows`/`fetchRecentlyWatched` | OPEN | n.v.t. | Gevonden tijdens de OFF-1-mergeronde (14 september), lokaal gereproduceerd op een branch die main_screen.dart/tv_top_navigation niet raakt: dus een bestaande regressie op `main`, niet door OFF-1 veroorzaakt. `MediaServerClient` heeft ergens deze drie methodes gekregen; de test-eigen `_FakeClient` (noSuchMethod-stub, `test/goldens/tv_discovery_landing_production_golden_test.dart:194`) is niet meegroeid, dus de discover-rijen blijven leeg en de golden wijkt 20.94% af. Blokkeert de Unit Tests-CI-check op elke PR tegen main tot de fake bijgewerkt is |
| OFF4 | Code Analysis-CI faalt op main op `avoid-unnecessary-setstate` in `lib/screens/seerr/seerr_discover_screen.dart:184`, niet reproduceerbaar lokaal | OPEN | n.v.t. | Gevonden tijdens de OFF-1-mergeronde. Zowel losse-file- als volledige-project-`flutter analyze` op de gepinde 3.44.0-SDK lokaal geven "No issues found"/geen warning-niveau treffers (58 info-only issues); de Linux CI-runner geeft deze warning wel, twee keer op rij (ook na een herrun). Matcht de bekende flaky-dart_code_linter-notitie, maar dan aanhoudend voor deze regel i.p.v. incidenteel: nader te onderzoeken of het lint-cache- of platformafhankelijk is |

...
| 14 | Instellingen | CODE/SIM CLOSED · VERIFY/SIM OPEN | `settings_screen.dart` herindeling naar northstar 14 (drie kaarten), `downloads_settings_screen.dart` (nieuw), `settings_utils.dart` gedeelde downloadlocatie-dialoog, DEC-115, `17ec3936` | widgettests groen (`test/screens/settings/`, `downloads_settings_screen_test.dart`, `setting_navigation_tile_test.dart`, TV-regressie in `tv_libraries_screen_test.dart` meegecheckt); geen ios-sim Verify-run tegen `14-instellingen.png` vastgelegd | n.v.t. | 8 (I7) |

...
| 4 | I5 bronkeuze (08) en contextmenu (09) | `UnifiedActivationCoordinator` beslist; sheets via de bestaande `OverlaySheetHost`; markeer-bekeken met groepssemantiek | geen tweede sheet-stack; contracttests. Gemerged in `main` via `025b400b` (contextmenu) en `68f98bbb` (bronkeuze/Wijzigen/failure-re-entry), widgettests groen. **09 `CODE CLOSED · VERIFY/SIM CLOSED (met bevinding)`** (18 sep, `e01fe36b`): `ios.contextmenu.northstar.yaml` PASS op een echte long-press (nieuwe `holdMs`-capability in de Verify-transport); de mobiele sheet mist de posterheader en drie rijen die TV al heeft, gedetailleerd in het register, niet gefixt (menu-uitbreiding, geen Verify-taak). **08 blijft `CODE CLOSED · VERIFY/SIM OPEN`**: de picker vraagt twee echte serverId's, wat de huidige Verify-transport niet kan zonder nieuwe testinfrastructuur, zie het register |
| 5 | I6 film- en seriedetail (06, 07) | gedeelde data, identity en activation, eigen mobiele widget; volledige synopsis; seizoenstaat blijft bewaard | widgettests en ios-sim Verify. **`CODE CLOSED · VERIFY/SIM CLOSED (met bevinding)`**: gemerged in `main` via `6c86a335` (14 sep, initiële bouw) en `309f6a82` (18 sep, ronde B regressiefixes). Widgettests groen, `ios.detail.northstar.yaml`/`ios.detail-episodes.northstar.yaml` beide PASS. De eerder gevonden afwijking, een losse volle-breedte "Downloaden"-knop naast "Afspelen" bij een serie waar de mockup alleen een download-icoon per episoderij toont, is in ronde B gefixt: `mobile_detail_view.dart` gate't die CTA nu op `!metadata.isShow`, zie register 06/07 |
...
| 7 | T3a fixtures | drie van de vier gebouwd: `seerr.requests.v1`, `catalog.long-rails.v1` (beide gesloten), `activity.active-session.v1` (klaar, geblokkeerd op een productbesluit); `catalog.watchlist.v1` niet gebouwd | VER4 en REQ1 dicht op de simulator (`f5fda145`..`08120ee2`, `f72466f2`..`2be95338`). WL2 is ACCEPTANCE GAP: de naad rond `WatchlistSourceFactory` bestaat en is gedekt op unit-niveau, het gat zit op het bevroren `/v1`-transport dat geen watchlist of favorieten kent, en de dekking hoort daarmee op een echte Jellyfin-aanmelding, de meting die de WL2-rij van de correctieronde vastlegt (TV2, 18 september). ACT1 blijft open: `MultiServerManager.isOwnerOrAdmin` blokkeert de Tautulli- en Activiteit-tegel voor elk Pleya-Server-only profiel, los van de fixture; de fake Tautulli-server (`f72466f2`, `0aa71164`) staat klaar voor zodra die poort een besluit krijgt. MYP1 blijft daarom open, toegewezen aan T2b-child MOC-16 voor de activiteitshelft (dat child noemt deze zelfde bevinding al) en aan TV2 voor de kijklijsthelft, dat WL2 als verificatie-infrastructuurgat behandelt, niet aan I7-child 18 |
| 8 | I7 Mijn Pleya (umbrella) | children, elk met eigen branch en tussentijdse merge: 18, 11, 12, 13 (projectie van bestaande events), 14, 15 (mobiel ontworpen, niet de TV verkleind), 19, 21 | elk child los naar `CODE/SIM CLOSED`. **18 `CODE/SIM CLOSED (met bevindingen)`** (18 sep, `99aab374`): het scherm had geen automation-ids, nu hergebruikt het de bestaande TV-ids; `ios.mypleya.northstar.yaml` PASS op de echte tik op `nav.myPleya`. Twee echte, nergens gebouwde bevindingen (de Plex-familie-waarschuwing en de Servers-statuspunt), gedetailleerd in het register, niet gefixt. **19 `CODE CLOSED · VERIFY/SIM CLOSED (met bevindingen)`** (20 sep, `c3d0504f` plus reviewronde `05466190`): de sectie "Mijn aanvragen" ververst nu bij een ander account of na een aanvraag en toont een foutregel met opnieuw proberen in plaats van te verdwijnen; een wachtende aanvraag heet niet meer "Goedgekeurd"; de kop opent voor een manager de eigen aanvragen; `/v1/open` wacht op de profielsessie. `ios.requests.northstar.yaml` PASS, verschillen met `19-aanvragen.png` (posters, telling, belicoon) staan in het register. **15 `CODE CLOSED · VERIFY/SIM CLOSED`** (18 sep, `8c80b2df`): de picker had ook geen enkele automation-node; nieuwe ids `screen.library_picker`/`library.picker.card[<index>]`; `ios.libraries.northstar.yaml` PASS langs de echte route (tik `nav.myPleya`, dan `my_pleya.tile[libraries]`). Screenshotvergelijking tegen `15-bibliotheken.png` toont geen nieuwe afwijkingen (de ontbrekende serverfilterchips volgen uit de fixture's ene server, "Alles weergeven" i.p.v. "Openen" was al DEC-114). **14 `CODE CLOSED · VERIFY/SIM CLOSED (met bevinding)`** (18 sep, `88a438e1`): de hoofdlijst had ook geen automation-ids; nieuwe `screen.settings`/`settings.tile[<slug>]`; `ios.settings.northstar.yaml` PASS langs de echte route (tik `nav.myPleya`, dan `my_pleya.tile[settings]`). Eén echte, onbeantwoorde bevinding: een "Bibliotheekzichtbaarheid"-tegel die northstar 14 niet toont, gedetailleerd in het register, niet gefixt (geen DEC beantwoordt de vraag). Koppelingen/Verbindingen staan buiten beeld op de simulator; deze Verify-DSL heeft geen scroll-stap, hetzelfde transportgat als I7-08. **21 `CODE CLOSED · VERIFY/SIM OPEN (ACT1 ACCEPTANCE GAP)`** (20 sep, `cd40a532`/`8c3ffd0a`): de goedgekeurde mobiele Activity-sheet heeft nu ids en een scenario; widgettests zijn groen en de echte simulator-run bouwt, installeert, logt in en bereikt Home, maar `activity.button` blijft afwezig omdat een Pleya-Server-only profiel nooit door `isOwnerOrAdmin` komt. Dit is de al geregistreerde ACT1-beslissing uit rij 7; geen predicate versoepeld of fixture gefaket. 11, 12 en 13 blijven `OPEN` |

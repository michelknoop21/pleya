...
| 4 | I5 bronkeuze (08) en contextmenu (09) | `UnifiedActivationCoordinator` beslist; sheets via de bestaande `OverlaySheetHost`; markeer-bekeken met groepssemantiek | geen tweede sheet-stack; contracttests. Contextmenu (09) **`CODE/SIM CLOSED · HARDWARE OPEN`**: gemerged via `025b400b`, groene ios-sim Verify-run (`fbec2f64`), onderweg een echte tap-target-fix (40→42px) gevonden en gedaan. Bronkeuze (08) blijft **`CODE CLOSED · VERIFY/SIM OPEN`**: gemerged via `68f98bbb`, widgettests groen, ios-sim Verify definitief geblokkeerd door DEC-063 (Pleya Server-bronnen groeperen nooit onderling), geen openstaand actiepunt, zie register rij 08 |

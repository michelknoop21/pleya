...
## DEC-096: Taalvoorkeuren in vier lagen, met het Pleya-profiel als eigenaar

**Date:** 2026-09-04
**Status:** accepted
**Context:** LANG1 in `docs/tvos-fysieke-correctieronde.md`, sectie G van de personalisatie-opdracht van 4 september. Het per-serie taalgeheugen van 17 augustus (`TrackPreferenceStore`, de sticky-prioriteit in `TrackSelectionService`, de Plex-spiegeling) dekt de gevraagde journey grotendeels, maar de code legt drie dingen niet vast. De speler geeft bij de overgang naar de volgende aflevering de spelende track door als navigatiekeuze, en die staat boven de sticky keuze, dus een tijdelijke terugval wordt binnen de sessie permanent. De sleutel is een ratingKey op één server, terwijl hoofdstuk 14.8 de bronvoorkeur al op de canonieke identiteit sleutelt. En er is geen beheer: geen overzicht, geen terugzetten. Het voorstel van 4 september zette het serverprofiel als globale eigenaar; Michel heeft dat in de beslissingsronde van dezelfde dag afgewezen, omdat de eis luidt dat de voorkeur voor álle content geldt, ook over servers en backends heen. Dit besluit is de gecorrigeerde versie.
**Decision:** (1) **Vier lagen, in deze volgorde.** Een uitdrukkelijke keuze van de kijker tijdens de lopende playback; de serievoorkeur; de globale voorkeur op het Pleya-profiel; de terugval op bron en bestand. (2) **"Uitdrukkelijke keuze" is uitsluitend een echte handeling van de kijker.** Het is niet de spelende track, niet een automatisch gekozen terugval, niet een track die uit de vorige aflevering is doorgegeven, en niet een bron-default die toevallig aanstond. Iedere nieuwe aflevering resolveert de intentie opnieuw uit de lagen hierboven. (3) **Het terugvalcontract.** Audio: is de gewenste taal of de originele taal aanwezig, dan die; anders tijdelijk de bron- of standaardtrack. Ondertitels: is de gewenste taal aanwezig, dan die; anders de ingestelde terugvaltaal als die beschikbaar is; anders uit. Nooit "de eerste beschikbare ondertiteltrack". De terugvaltaal is een echte profielvoorkeur (`subtitleFallbackLanguage` op het Pleya-profiel, in de UI "Terugvaltaal ondertitels"), geen hardgecodeerd Engels; hij wordt bij eerste gebruik geseed uit de gerangschikte lijst van het serverprofiel waar die bestaat, en is daarna zelfstandig te wijzigen. Een terugval is tijdelijk: hij wijzigt de serievoorkeur niet, hij wijzigt de globale voorkeur niet, en hij reist niet mee naar de volgende aflevering. (4) **Geen driekeuzevraag.** Staat "Onthoud keuzes per serie" aan en wisselt de kijker bewust van audio of ondertitel, dan wordt dat direct toegepast, wordt de serievoorkeur bijgewerkt en bevestigt een korte toast dat. Staat de schakelaar uit, dan geldt de wissel alleen voor die playbacksessie en ontstaat er geen serie-override. De globale voorkeur verandert nooit automatisch vanuit de speler; die wijzig je uitsluitend in Mijn Pleya ▸ Instellingen ▸ Taal en ondertitels. (5) **Het Pleya-profiel is de eigenaar van de globale voorkeur.** `PleyaProfilePlaybackLanguagePreferences` draagt minimaal de audiovoorkeur, de ondertitelvoorkeur, het ondertitelbeleid, de terugvaltaal en `rememberPerSeries`. Serverprofielen zijn een spiegel, nooit de autoriteit. (6) **Spiegelen naar Plex blijft, capability-gated.** Het Pleya-profiel is canoniek, Plex is een optionele spiegeling waar de backend hem ondersteunt, en een andere backend krijgt zijn eigen capability als hij die later krijgt. Een mislukte schrijfactie naar Plex maakt de Pleya-voorkeur niet ongeldig en rolt niets terug. (7) **De serievoorkeur hoort bij de logische serie waar de identiteit betrouwbaar is.** Dezelfde logische serie op NAS, Zolder of een andere server deelt dan één voorkeur. Een onterechte samenvoeging is erger dan een gemiste: is er geen betrouwbare cross-source identiteit, dan valt de sleutel terug op de concrete server-en-serie-sleutel, en er komt geen samenvoeging op alleen titel en jaar bij die vandaag niet bestaat. De bestaande serversleutel blijft het migratie- en terugvalpad. (8) **Nooit track-id's bewaren.** Opgeslagen wordt de intentie: taal, de intentie "originele taal", en het ondertitelbeleid. Niet een trackindex, een track-id of een stream-id van één aflevering. Elke concrete playback resolveert opnieuw tegen de beschikbare tracks. (9) **Eén beheerplek.** Mijn Pleya ▸ Instellingen ▸ Taal en ondertitels toont de globale voorkeur met eigenaar, de serievoorkeuren met herkomst, en per serie de actie "Gebruik globale voorkeur", die de hele serie-override wist waarna het Pleya-profiel direct weer geldt. De taalschakelaars die nu onder Afspelen staan verhuizen hierheen; er blijven geen twee onafhankelijke eigenaren bestaan. Heeft het spelerinfopaneel later een beheeringang nodig, dan linkt het naar deze pagina in plaats van dezelfde instelling opnieuw lokaal te bouwen. (10) **De toast is een presentatiecontract.** Hij pakt geen focus, blokkeert geen afstandsbedieningsinvoer, verdwijnt vanzelf en staat in de bestaande spelerzone: `PlayerToastIndicator` in de `Positioned.fill` met `IgnorePointer` uit `video_controls.dart`, boven uitgelijnd. Ondertitels staan onderaan onder `SettingsService.subtitlePosition`, dus die zone raakt ze per constructie niet. Geen zelf gekozen Y-positie. Mockup 31 A tot en met D is bindend voor de compositie.
**Consequences:** `TrackSelectionService` krijgt een intentie in plaats van een doorgegeven track als prioriteit 1, en `episode_navigation.dart` geeft die intentie door in plaats van `currentAudioTrack` en `currentSubtitleTrack`. `TrackPreferenceStore` krijgt een logische sleutel met de serversleutel als terugval en een migratie bij de eerstvolgende schrijfactie; `TrackLanguageChoice` krijgt een manier om een veld te wissen zodat een regel weer leeg kan worden. Er komt een profielvoorkeurenmodel op het Pleya-profiel met een seed uit het serverprofiel, en de Plex-schrijfactie wordt een capability-gated spiegeling die bij mislukking alleen logt. `PlayerToastIndicator` krijgt een tweede regel en een langere standaardduur en blijft in zijn bestaande zone. De pagina is een nieuwe Instellingen-subpagina in de bestaande geneste route, en `playback_settings_screen.dart` verliest de twee taalschakelaars. De negatieve controles staan als CONTROL A tot en met I onder LANG1 en draaien rood voordat er een regel productiecode verandert. Wat dit besluit niet doet: het raakt de bronvoorkeur van hoofdstuk 14.8 niet, het verandert de identiteitsregels van hoofdstuk 11 niet, en het introduceert geen nieuwe samenvoegingsregel om taalvoorkeuren te kunnen delen. Goedgekeurd door Michel op 4 september 2026 in de beslissingsronde LANG1 / mockup 31, met de tien correcties hierboven als bindend.

## DEC-097: "Recent uitgebracht" is een venster van 90 dagen op de releasedatum, en zonder datum hoort een titel er niet in

**Date:** 2026-09-05
**Status:** accepted
**Context:** HERO3 in `docs/tvos-fysieke-correctieronde.md`. Op de Apple TV (build 251) toonde de Home-hero films die niet recent uitgebracht waren. DEC-067 punt 2 en hoofdstuk 9.5 zeggen "uitsluitend recent uitgebrachte films", maar definiëren "recent" niet, en `getLatestMoviesFromAllServers` implementeerde alleen "op releasedatum geordend": de 100 laatst toegevoegde items, films eruit, sorteren, top 12. Niets weigerde een film uit 1998, en een film zonder releasedatum reed mee op `addedAt`. Voor een Pleya Server, een Pleya Share en een lokale map, die geen releasedatum over de lijn dragen, was de hero daardoor letterlijk "recent toegevoegd" onder de naam "recent uitgebracht".
**Decision:** (1) Een film hoort in de pool van "Recent uitgebracht", en dus in de hero en in de rij van die naam, als zijn releasedatum ten hoogste 90 dagen vóór nu ligt. De grens is inclusief. (2) Een film zonder releasedatum hoort er niet in, hoe recent hij ook is toegevoegd: "recent uitgebracht" is een uitspraak over de datum, en zonder datum is die uitspraak niet te doen. (3) Is de pool daardoor leeg, dan geldt het bestaande fallbackgedrag van DEC-067 (Verder kijken, dan hubs) en wordt er niet aangevuld uit oudere films. (4) Het venster heeft één eigenaar, `DataAggregationService.heroReleaseWindow`; `FeaturedSelector` herhaalt het niet.
**Consequences:** Pleya Server, Pleya Share en de lokale map leveren vandaag geen film aan de hero, omdat het `/v1`-contract en de andere twee geen releasedatum dragen. Dat is een protocolgat, gemeld naast het identiteitsgat van LANG1, voor de fase die het contract mag wijzigen; het wordt niet stil opgelost met `year` of `addedAt`. De Verify-fixture is een Pleya-fake-server, dus de hero-scenario's op de simulator zien de fallback in plaats van een film; wat ze bewijzen verschuift daarmee van "de hero rouleert" naar "de fallback staat". Doorgevoerd op 6 september 2026: `discover.hero.layout` en zijn macOS-variant zijn `discover.layout` en `discover.layout.macos` geworden en eisen `hero_visible == false` op `discover.continue_watching` (VER-CI in `docs/tvos-fysieke-correctieronde.md`). `test/services/data_aggregation_bridge_test.dart` legde het oude gedrag vast en is bijgewerkt; de HERO3-controles eisen de grens. Besluit van Michel, 5 september 2026, uit twee keuzes: 90 dagen boven 180 of 365, en "buiten de hero" boven "toegevoegd-binnen-venster als plaatsvervanger".

## DEC-098: Pleya Verify krijgt een `walk`-stap met een orakel dat alleen meldt wat de traversal niet had kunnen kiezen

**Date:** 2026-09-05
**Status:** accepted
**Context:** WALK in `docs/tvos-fysieke-correctieronde.md`. Elk scenario benoemde met de hand waar een druk hoort te landen (`press` gevolgd door `wait_until focused:`), dus focussprongen werden geval voor geval gevonden nadat iemand ze op het toestel tegenkwam. `discovered`-knopen in `/v1/ui_tree` hadden geen identiteit: `label` is `FocusableWrapper` voor het grootste deel van de app, `focused` is waar voor de hele voorouderketen, en een rect beweegt onder de druk die beoordeeld wordt.
**Decision:** (1) `walk` is een stap, geen assert-predicaat: een sprong is twee frames met een druk ertussen. (2) `AutomationRegistry` geeft elke `FocusNode` een procesnummer uit een `Expando` en draagt dat als `node` op declared, discovered en `/v1/focus`; additief, de runner valt terug op rects. (3) Het orakel (`focus_walk.dart`, puur) is geen kopie van `DirectionalFocusTraversalPolicy`; het houdt de geschiktheidstest en de band van de bron over, zodat het niets meldt wat het beleid nooit had kunnen kiezen. (4) Intentie gaat via het scenario: `expect` benoemt een landing, `allow` een kandidaat die gepasseerd mag worden; marges worden niet verruimd.
**Consequences:** 41 vectoren in `pleya_verify/focus_walk/cases.json`, twaalf engine-tests, acht validator-tests, runner-suite 320 groen (`c4ffcd16`). De stap vangt de dubbele druk uit de aanraaklaag niet: idb stuurt één druk zonder aanraakstroom, dus een groene walk over de navbalk bewijst NAV1 niet. Nog open: `nav.profile-id` op de profielchip, de vier scenario's op de simulator, de twee sabotagecontroles en de docs.

## DEC-099: De tvOS Menu-passthrough wordt pas aangezet als de remote los is, en de engine-fork wordt gelezen vóór er gebouwd wordt

**Date:** 2026-09-05
**Status:** accepted
**Context:** NAV1 in `docs/tvos-fysieke-correctieronde.md`. Acht toestelbuilds en vier logs zochten de dubbele pijlstap in de UIPress-fasen. De gepinde engine (`github.com/edde746/flutter-tvos`) is een patchreeks, geen bron, en was nooit gelezen. Gereconstrueerd staat er dat `setMenuPressPassthroughEnabled:YES` elke vastgehouden toets loslaat (`releaseAllSynthesizedPresses`) en dat `.ended` een losgelaten toets opnieuw tikt (`tapIfMissingKeyDown:YES`). De app zet die vlag precies op de druk die op de Home-tab landt.
**Decision:** (1) `TvosSystemNavigationService` stuurt een enable pas als `HardwareKeyboard.physicalKeysPressed` leeg is, op de eerstvolgende key-up; een disable gaat direct. De fix zit bij de afzender van het kanaalbericht, niet in `AppDelegate` en niet in een Dart-heuristiek. (2) `scripts/tvos_engine_source.sh` reconstrueert de engine-bestanden uit de patchreeks; `docs/tvos-remote-press-pipeline.md` legt het pad per station vast met een symptoomtabel en een meetprotocol. Een remote-melding begint daar, niet bij een build. (3) Een meetlog voor remote-input bevat naast fase en toets ook de kanaalberichten die de app in datzelfde venster verstuurt.
**Consequences:** Één pijldruk is weer één stap zodra build 259 dat op het toestel bevestigt. Menu kan een key-up later dan voorheen bij het systeem komen; dat is onmerkbaar, want Menu wordt niet ingedrukt terwijl een pijl vastzit. Een fout in het engine-contract zelf blijft een patch in de reeks plus een eigen engine-build en een bump van `tvos/engine.version`; dat pad is beschreven en bewust niet als eerste gok genomen. Pleya Verify kan de engine-helft niet raken (een idb-druk bereikt de swizzle-hook niet, gemeten); het app-contract wordt bewaakt door `tvos.nav.held-press-lands-once` op de state-node `tvos.menu_passthrough`, met een negatieve controle waarin de deferral uitstaat.

## DEC-100: Eigen rijen op Home komen uit een bewaard filter, en Home laat zich vanuit Home zelf herschikken

**Date:** 2026-09-05
**Status:** accepted
**Context:** ROW1 in `docs/tvos-fysieke-correctieronde.md`. Michel op 5 september: "het idee is dat je zelf rails aan kan maken door middel van een filter toe te passen en dan de inhoud van de filtering in de rails weer te geven", "op de homescreen zelf" en "op een mooie manier"; de hero en Verder kijken "blijven wel statisch". Vandaag komt elke rij op Home uit een hub van de backend (`homeRowId` is `serverId:identifier`), `HomeLayoutProvider` bewaart per profiel alleen wat verborgen is en in welke volgorde, en `HomeLayoutScreen` doet dat onder Instellingen. Michel stuurde dezelfde dag een functieschets met zes schermen en twee eisen: "alle knoppen in tvos goed bereikbaar" en "puur de functie", niet de styling. Mockup 32 tekent die functie in de tokens van de set (`docs/assets/tvos-unified/mockups-2026-09-04/32-eigen-rails-*.png`). De eerste versie zette de ingang in de CTA-rij van de hero en de filterkeuzes als tags naast het raillabel; Michel schrapte beide, de ingang omdat hij de slidewissel van 7.3 kost en niet goed te bereiken is, de tags omdat "actieve filters niet weergeven hoeven te worden boven de rails".
**Decision:** (1) **Een eigen rij is een bewaard filter met een naam.** Soort (films of series), een `UnifiedCatalogFilterSelection`, een `UnifiedCatalogSort` en een optionele naam; is de naam leeg, dan volgt hij het filter en verandert mee. De inhoud is wat de catalogus van hoofdstuk 10 op dat filter teruggeeft, in die sortering. De rij is per profiel en staat naast de bestaande hide- en orderlijst van `HomeLayoutProvider`, met een eigen rij-id dat niet met `serverId:identifier` kan botsen. (2) **In rust is een eigen rij van een hubrij niet te onderscheiden.** Alleen zijn naam als label, geen tags, geen filtersamenvatting; de laatste kaart is een tegel "Alle N, in Alle films" die de catalogus met ditzelfde filter opent, de tegelvorm die DEC-064 toestaat. (3) **De ingang raakt de hero niet.** "Home aanpassen" staat als voetregel over de volle breedte onder de laatste rij, één DOWN vanaf die rij zonder horizontaal mikken (A1a), én als laatste regel in het contextmenu van elke kaart op Home, achter een scheidingslijn (A1b). 7.3 verandert niet. (4) **Het bewerken is één paneel over de gedimde Home** (B). Uitgelicht en Verder kijken staan bovenaan met een slot en het woord "vast" en zijn niet focusbaar; een hubrij krijgt omhoog, omlaag en Verbergen, een eigen rij omhoog, omlaag, Bewerken en Verwijderen, een verborgen rij staat gedimd met Tonen, onderaan een gestippelde regel "Nieuwe rij". Elke regel is een horizontale focusgroep, UP en DOWN houden de kolom vast, vaste regels worden overgeslagen, een uitgeschakelde pijl blijft focusbaar zodat de kolom niet springt. Verplaatsen gaat per stap met de pijlknoppen, niet met optillen. Klaar rechtsboven is UP vanaf de eerste regel; Menu doet hetzelfde. Wijzigingen gelden direct, er is geen aparte opslagstap. (5) **Een nieuwe rij in drie stappen** (C1 tot en met C3): naam en soort, filters met Sortering als vijfde regel, en een voorbeeld met teller en de eerste posters. De rij landt direct onder Verder kijken en het paneel sluit op Home met de focus op de eerste kaart van de nieuwe rij. (6) **Lege en korte uitkomsten.** Levert het filter niets op, dan is Rij toevoegen uitgeschakeld, kleurt de teller amber en staat de focus op Filters aanpassen (C4). Onder de zes titels blijft toevoegen mogelijk; zo'n rij krijgt op Home "korte rij" naast het label. Een bewaarde rij die later leeg raakt verdwijnt in rust van Home en blijft in het paneel staan met "leeg" als subregel. (7) **De teller volgt 10.7:** exact zodra alle bronnen uitgeput zijn, anders "N geladen".
**Consequences:** 9.1, 17.5 en 23 zijn aangepast. `HomeLayoutScreen` onder Instellingen blijft bestaan voor pointerplatforms en krijgt de eigen rijen erbij; op TV wordt het paneel van (4) de enige ingang. De code verandert nog niet; de bouw is een eigen ronde. Wat die ronde raakt: een model voor het bewaarde filter met opslag per profiel naast `getHomeRowOrder`; `TvContentFeed._rows` dat eigen rijen tussen de hubrijen plaatst op de bewaarde volgorde; een catalogusvraag die de eerste kaarten en een teller levert zonder de volledige paging van hoofdstuk 12; de tegel "Alle N" die `TvUnifiedCatalogScreen` opent met het filter al gezet; het paneel, de driestapsflow en de contextmenuregel; en `HomeLayoutProvider.apply` dat een rij-id van een eigen rij kent. Negatieve controle: een widgettest op `TvContentFeed` die een eigen rij uit een bewaard filter tussen de hubrijen eist, met de hero en Verder kijken op hun vaste plek, rood op de huidige code. Wat dit besluit niet doet: het verandert de filtervelden van 10.4 niet (bekeken en actief bezig blijven buiten het neutrale query-model, zie `UnifiedWatchFilter`), en het raakt de hubsemantiek van 17.2 niet. Goedgekeurd door Michel op 5 september 2026 op A1a, A1b, A2, B en C1 tot en met C4: "Ja dit is top akkoord". **Gebouwd op 6 september 2026** in `040c939a` (het model, de opslag per profiel, de loader en de rij op Home) en `db1bc994` (de twee ingangen, het bewerkpaneel en de driestapsflow). Twee dingen uit dit besluit staan daar bewust naast en hebben hun eigen regel in `docs/tvos-fysieke-correctieronde.md`: de tegel "Alle N" als laatste kaart van een eigen rij (ROW1c), omdat die een extra tegel in `TvDiscoveryRail` is en daarmee zijn `itemCount`, zijn randstop, zijn semantics en zijn goldens raakt; en de pointerkant (ROW1b), waar `DiscoverScreen` een eigen rijenpad heeft en `HomeLayoutScreen` op de pre-unified `homeRowId`-ruimte ordent. Eén afwijking van de tekst hierboven is bij de bouw gebleken en is doorgevoerd: Recent uitgebracht gaat mee de layout in, want mockup 32 B tekent hem met dezelfde verplaats- en verbergknoppen als een backendrij en punt (4) zet precies twee rijen vast.

## DEC-101: Het infopaneel is op TV het enige spelermenu, en mockup 33 is zijn taal

**Date:** 2026-09-05
**Status:** accepted
**Context:** PLR2 in `docs/tvos-fysieke-correctieronde.md`. Michel meldde dat de geluidsinstellingen in het veeg-omlaag-menu op de Apple TV niet goed leken te werken. De audit vond drie spelermenu's naast elkaar op TV (`TvInfoPanel` na een veeg, de 10-foot `VideoSettingsSheet` achter het tandwiel, de `TrackSheet` achter de sporenknop) die elkaar voor de helft overlappen en tegenspreken; een "Maximum volume"-rij die op TV alleen het plafond `volume-max` verhoogt terwijl de volumeregelaar daar verborgen is, dus hoorbaar niets doet (AUD1); een synchronisatie-subweergave die opent zonder gefocust element omdat `SyncOffsetControl._buildFull` de `sliderFocusNode` niet koppelt (AUD2); en een paneel zonder Select op de pills, zonder LEFT/RIGHT op waarderijen, zonder automation-ids en zonder tests (PNL2). Mockup 19 (PB-9) bevatte een Wachtrij-pill en een schakelaar "Onthouden voor deze titel" die met PB-9 en DEC-096 botsen, gaf de geluidsuitvoer geen plek, kopieerde de globale stijlpagina half en tekende één stand. Michel op 5 september: het paneel wordt het enige menu, een werkende volumeboost, mockup 19 opnieuw, de video speelt door, vier tabs.
...
## DEC-107: Een waarderij van het spelerpaneel wordt eerst aangeklikt, en pas dan stappen links en rechts

**Date:** 2026-09-06
**Status:** accepted
**Context:** PLR5 in `docs/tvos-fysieke-correctieronde.md`. Michel op 6 september, op een fysieke Apple TV met build 264: vanaf Beeldverhouding kom je de linkerkolom niet uit zonder de beeldverhouding te veranderen. Dat klopt met de code. `TvPanelRow` geeft `onStepLeft` en `onStepRight` door aan `FocusableWrapper`, en die consumeert LEFT en RIGHT zodra er een callback hangt; `clampedSteps` levert alleen aan de uiteinden van de waardenlijst `null`, zodat de toets daar valt en de traversal de kolom mag verlaten. Staat de waarde in het midden, dan bestaat er geen weg naar de rechterkolom die de waarde ongemoeid laat. De linkerkolom van het tabblad Video heeft vijf zulke rijen, en drie ervan (beeldverhouding, zoom, sfeerverlichting) staan standaard midden in hun lijst. Michels voorstel is een rij die je eerst aanklikt: binnen die stand verstellen LEFT en RIGHT de waarde, daarbuiten navigeren ze tussen de kolommen. Dat is een ander interactiemodel dan punt (4) van [DEC-101](#dec-101), waarvan de voetregel letterlijk "Links en rechts stappen een waarde" belooft, dus het gaat als besluit en niet als stille correctie.

**Decision:** (1) **Een waarderij heeft twee standen.** In rust is een gefocuste waarderij een gewone rij: LEFT en RIGHT bereiken hem niet, de traversal krijgt ze, en de ring kan de kolom verlaten waar de waarde ook staat. Aangeklikt stapt LEFT de waarde omlaag en RIGHT omhoog, met dezelfde klem aan de uiteinden die `clampedSteps` nu al oplevert: aan het eind gebeurt er niets, de ring verlaat de rij niet. Alleen rijen met `onStepLeft` of `onStepRight` kennen deze stand; chevronrijen en schakelaars veranderen niet. (2) **Select wisselt de stand.** Select op een waarderij in rust klikt hem aan; Select op een aangeklikte rij laat hem los. Daarmee vervalt het huidige "Select cyclet vooruit" van DEC-101 (4) voor waarderijen: één toets kan niet allebei. Wie een lijst wil doorlopen doet dat met RIGHT, dat aan het eind stopt in plaats van rond te lopen; dat is een gedragswijziging en hij staat hier expliciet zodat hij niet later als bug terugkomt. (3) **Er zijn drie uitgangen, want een stand waar je in vast kunt komen te zitten is precies de klasse fout waar PLR6 over gaat.** Select laat los, Menu laat los, en UP of DOWN laat los én verplaatst de ring in één druk. Menu sluit het paneel alleen wanneer geen enkele rij is aangeklikt; dat is dezelfde laagsgewijze Menu die DEC-101 (3) voor subweergaven vastlegt, nu met een derde laag erboven. **Dit is de zwakke plek van dit voorstel** en de reden dat het naast PLR6 ligt: het maakt de weg naar buiten één druk langer op precies het oppervlak dat op hardware al niet te verlaten was. (4) **De stand is zichtbaar.** Een aangeklikte rij toont pijlen links en rechts van zijn waarde; in rust staan die er niet. Zonder dat verschil is de rij een `SegmentedButton` in `monoTheme`: de stand werkt en je ziet hem niet ([DEC-053](#dec-053)). (5) **De voetregel vertelt welke stand geldt.** In rust: "Choosing applies right away · Select opens a value row · Menu closes the panel". Aangeklikt: "Left and right step the value · Select or Menu is done". `hintBack` van een subweergave verandert niet. De Engelse basislocale krijgt daarvoor één nieuwe sleutel, `videoControls.tvPanel.hintValueRow`, en `hint` wordt herschreven; de veertien vertalingen vallen tot ze bijgewerkt zijn terug op het Engels, zoals `slang` dat altijd doet. (6) **De stand is per paneel, niet per rij.** Hoogstens één rij tegelijk is aangeklikt; een rij die de focus verliest laat automatisch los, zodat er geen aangeklikte rij buiten beeld achterblijft.

**Consequences:** `TvPanelRow` krijgt de stand en gaat van stateless naar stateful, of de stand komt bij `_TvInfoPanelState` te liggen met de gefocuste node als sleutel; dat laatste past beter bij (6) en houdt de rij dom. `FocusableWrapper` mag LEFT en RIGHT alleen nog consumeren wanneer de rij aangeklikt is, dus `onNavigateLeft` en `onNavigateRight` worden voorwaardelijk doorgegeven in plaats van vast. Het aanklikken en het loslaten moeten allebei `SelectKeyUpSuppressor.suppressSelectUntilKeyUp()` wapenen: op tvOS levert de engine KeyDown en KeyUp in één callback, en zonder wapening klikt één druk de rij aan en meteen weer los. De bestaande test `a value row clamps at its ends so the ring can leave the column (PNL2)` verandert van betekenis en moet worden gesplitst: één test dat een rij in rust LEFT laat vallen zodat de ring de kolom verlaat, en één dat een aangeklikte rij aan zijn uiteinde niets doet én de ring vasthoudt. De negatieve controle is de melding zelf: met de focus op Beeldverhouding, midden in zijn lijst, moet RIGHT de rechterkolom bereiken zonder dat de beeldverhouding verandert. Die test is rood op de huidige code. Elke waarderij van de drie tabbladen wordt geraakt (beeldverhouding, zoom, sfeerverlichting, afspeelsnelheid, volumeversterking, prioriteit, tekstgrootte, achtergrond), plus de subweergave voor synchronisatie, waarvan de eigen voetregel `syncStepHint` al een ander model beschrijft en die daarom buiten dit besluit blijft. DEC-101 (4) wordt door dit besluit vervangen voor waarderijen; de rest van DEC-101 blijft staan. Goedgekeurd door Michel op 6 september 2026: "Akkoord, zo bouwen". **Gebouwd op 6 september 2026** in `08814bac`. Eén afwijking van de tekst hierboven is bij de bouw gebleken en is doorgevoerd: het wapenen van `SelectKeyUpSuppressor` bij het aanklikken en loslaten is er niet gekomen, want `handleOneShotSelect` voert Select al uit op de KeyDown en consumeert de KeyUp, dus één druk klikt één keer; een tweede suppressor wapenen zou juist de vastgepinde onderdrukking opleveren waar `handleBackKeyAction` voor waarschuwt. Toegevoegd is `entersOnSelect`, de opt-out die de synchronisatie-subweergave gebruikt: één rij op een eigen pagina, geen kolom om te verlaten, en `syncStepHint` belooft een directe stap. De cyclus-callbacks en `stepValue` zijn met de cyclus mee verdwenen; `clampedSteps` en `stepValueClamped` blijven, nu als klem binnen een aangeklikte rij in plaats van als uitweg uit de kolom.

## DEC-108: Kijklijst, Aanvragen en Zoeken krijgen de catalogustaal, en Alle aanvragen wordt een raster
...
## DEC-116: Film- en seriedetail openen met een ingeklapte topnav, en detail reserveert onder de shell geen eigen bovenmarge meer

...
**Consequences:** Pleya Server, Pleya Share en de lokale map leveren vandaag geen film aan de hero, omdat het `/v1`-contract en de andere twee geen releasedatum dragen. Dat is een protocolgat, gemeld naast het identiteitsgat van LANG1, voor de fase die het contract mag wijzigen; het wordt niet stil opgelost met `year` of `addedAt`. De Verify-fixture is een Pleya-fake-server, dus de hero-scenario's op de simulator zien de fallback in plaats van een film; wat ze bewijzen verschuift daarmee van "de hero rouleert" naar "de fallback staat". `test/services/data_aggregation_bridge_test.dart` legde het oude gedrag vast en is bijgewerkt; de HERO3-controles eisen de grens. Besluit van Michel, 5 september 2026, uit twee keuzes: 90 dagen boven 180 of 365, en "buiten de hero" boven "toegevoegd-binnen-venster als plaatsvervanger".

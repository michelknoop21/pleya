...
| PS-5 DeviceCapabilities | in `feat/pleyaserver`, code complete | nee | nee | nee (spec h19) | nee | criterium 4 open, DEC-064 | zacht, open releasegate | N: hardwareronde vóór publieke release; blokkeert de slices niet |
...
| Catalogus | file, version, item, streams | zoals ontworpen; `content_fingerprint` dood | boeken passen niet in `media_*` (DEC-093) | S3: `publications` en `publication_files`; `media_*` blijft audiovisueel |
...
| Watch-state | DEC-049 lease en revisie | zoals ontworpen | leesvoortgang is geen kijkstatus (DEC-093) | S6: eigen `reading_states` (RB-4) |
...
| PS-4W | niet gestart, geknipt door DEC-073 | S13, met één extra mockup (speler) vóór de bouw |

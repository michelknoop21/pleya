...
| SYS-2 (BACK1), SYS-7, MOC-09/10 Verify, DET4 Verify, MOC-13/14/15/16 Verify, filters en sorteren, FOC1 (`8d5fa38a`), SYS-1d (`9abdaee5`), SEARCH2 (`dfde8799`), LAND6 (`f4bb816f`), LAND7 (`1f69a855`), WL3 (`9963071c`) | TV8 | de historische Verify-gaten; nieuw werk brengt zijn eigen scenario mee. FOC1 en SYS-1d zijn allebei navigatiegedrag: een journey hoort hier, niet bij TV1, want beide zijn een historisch gat in een al gebouwd oppervlak, geen oppervlak dat TV1 zelf bouwt. SEARCH2, LAND6, LAND7 en WL3 zijn dezelfde redenering vanuit TV2 (18 september): vier focus-/layoutbevindingen op al gebouwde oppervlakken, geen oppervlak dat TV2 zelf bouwt. DET4 (TV3, 19 september, `518dae18`) is dezelfde redenering: layoutgedrag op het al door MOC-10 gebouwde seriedetail-oppervlak, geen oppervlak dat TV3 zelf bouwt, dus zijn detail-journey (seriedetail, seizoenchips, de ring over minstens drie chips) hoort bij de MOC-09/MOC-10-detailjourneys hierboven. CTX1-3 vullen hetzelfde menu als MOC-12, maar krijgen hun journey al van TV3 zelf: `tvos.detail.context-menu.yaml` (`aa530095`), dus TV8 bouwt die niet opnieuw. WL2 gaat hier bewust niet bij: die dekking hoort op een echte Jellyfin-aanmelding (zie de WL2-rij in `tvos-fysieke-correctieronde.md`), en een scenario dat nooit kan slagen zou de `tvos.library.sort`-fout herhalen. TV4's eigen Live TV-journey (Gids, Nu op TV, Opnames, een kanaal openen) hoort hier evenmin bij, om dezelfde reden als WL2: de Verify-fixture is een Pleya Server en `/v1` heeft geen Live TV-endpoints, zie de `ACCEPTANCE GAP`-rij LIVE2 in `tvos-fysieke-correctieronde.md`. Dit is **geen** inhaalwerk dat TV8 alsnog kan doen: TV8 draait tegen dezelfde fixture en loopt op precies dezelfde grens vast. De journey wacht op een Jellyfin-fixture of op Live TV in het protocol na PS-8, en TV8 behandelt hem niet als ontbrekende closure, exact wat de §7-rij in dit hoofdstuk voor de hardware-open rijen al doet. Geleverd, zodat TV8 hem niet dubbel bouwt: `tvos.livetv.absent-without-capability.yaml` (zonder de capability staat er geen Live TV-pil in de bar) |
...
**Uitkomst (TV3 Task 8, 19 september):** CTX1-3 kregen hun journey van TV3 zelf,
`tvos.detail.context-menu.yaml` (`aa530095`), al geleverd. DET4 niet: het is layoutgedrag op het
al door MOC-10 gebouwde seriedetail-oppervlak, geen oppervlak dat TV3 zelf bouwt, dezelfde
redenering die TV1 voor FOC1/SYS-1d gebruikte en TV2 voor SEARCH2/LAND6/LAND7/WL3. De detail-journey
voor DET4 gaat daarom naar TV8, naast de bestaande MOC-09/MOC-10-detailjourneys, zie de TV8-rij in
hoofdstuk 3.
...
TV4-uitvoering (19 september): de exit is gehaald op tests, en waar de omgeving het toeliet op een
simulatorbeeld. De vier compositie-elementen van MOC-17 (paginakop, capsulerij, gidsdichtheid,
detailbalk) hebben elk een groene test, en LIVE1 heeft een negatieve controle die vóór de fix
aantoonbaar rood stond. Een simulatorbeeld ontbreekt voor de gids en de detailbalk: geen enkele
aangesloten demo-server had een werkende tuner. De gevraagde groene Verify-run draagt een
`ACCEPTANCE GAP` (LIVE2 in `tvos-fysieke-correctieronde.md`): de fixture is een Pleya Server, `/v1`
heeft geen Live TV-endpoints, en TV4 heeft in plaats daarvan het ene scenario geleverd dat tegen die
fixture wél kan slagen (`tvos.livetv.absent-without-capability.yaml`, groen op de simulator).

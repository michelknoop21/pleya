...
| SYS-1d | Twee TV-contentroutes pushen buiten `tvContentRouteRegistry` om (`tv_libraries_screen.dart:220`, `tv_content_feed.dart:369`) | PB-1 | CODE CLOSED · VERIFY/SIM OPEN | `cff2022b`, zie `tvos-fysieke-correctieronde.md:113`, geen Pleya Verify-scenario, hardwareronde open |
...
| SYS-3a | OVR1a: de schaalbasis van paneelinhoud op TV | PB-5 | GESLOTEN als koepel, OPGESPLITST | Inventarisatie in `docs/tvos-sys3a-schaalinventarisatie.md` (67 bestanden, 93 call sites op `scaleOf`/`scaleForSize`): 63 bestanden scheiden displayschaal (`scaleOf`, paneelbeschermd) en eigen doosgeometrie al correct, inclusief het SYS-3b-sheetpatroon. Twee bestanden doen dat niet en lezen hun hele schaal, inclusief lettergroottes, rechtstreeks uit `MediaQuery.sizeOf(context)`: `lib/widgets/tv_browse_rail.dart` (hoge zekerheid, gemonteerd onder de al bewezen DET1-nesting) en `lib/screens/libraries/tabs/library_recommended_tab.dart` (zelfde patroon, lagere zekerheid over nesting). Die twee verdienen elk een eigen ID in `docs/tvos-fysieke-correctieronde.md`, toegewezen aan het plan dat dat oppervlak bezit; niet in deze taak toegevoegd of gefixt. J3-contracttests (`test/utils/layout_constants_test.dart`, `test/widgets/tv/tv_unified_media_grid_test.dart`) blijven groen, geen schaalwaarde gewijzigd |

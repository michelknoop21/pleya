...
die geen Phase ID heeft.** Op `feat/ebooks` staat DEC-069 ("Mobiele primaire navigatie landt
...
> onbekende soort aan.
>
> De bewering was al onjuist op het moment dat [DEC-093](DECISIONS.md) werd geschreven. De audit
...
ladder view < download < manage uit DEC-065 §4 en de rolomzeiling voor owner en admin uit §2. Er zit
niets audiovisueels in.

## 3. Waarom de huidige roadmap daardoor niet meer klopt

**Er is een goedgekeurd clientbesluit zonder serverdrager.** DEC-069 op `feat/ebooks` is `accepted`,
...
Het PS-9-venster is open voor precies de zeven wijzigingen uit DEC-068 en sluit daarna. Een achtste
...
| Afhankelijkheden | PS-14, DEC-069 op `feat/ebooks` |
...
Wijziging 2 is de haak die DEC-069 nodig heeft: de client leest `capabilities.ebooks` en niet
...
Boeken-bestemming. Dat sluit aan op DEC-069 punt 5 (e) op `feat/ebooks`, dat tvOS en macOS al buiten
...
**Een eigen `ebook_permissions`.** Dat zou de ladder uit DEC-065 §4 dupliceren, met twee plekken
...
DEC-030 tot en met DEC-039, DEC-049, DEC-063 tot en met DEC-073, en DEC-091. Dat is geen incident
...
- de bestaande Pleya Server-**DEC-069** (`sid` door de authketen) **blijft staan**; er hangen
...
- dit roadmapbesluit krijgt **DEC-093**, het eerste nummer dat op geen enkele branch voorkomt

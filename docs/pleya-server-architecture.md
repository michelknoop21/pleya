...
| Het verlaten van de speler wacht op de afsluitrapportage van de kijkstatus. Loopt die in een connect-timeout, dan staat de gebruiker seconden naar een zwart scherm voordat de bibliotheek terugkomt. Gemeten op 21 augustus 2026 op iOS en tvOS tegelijk, beide op dezelfde tunnel: `POST /watch-state` gaf op allebei `connect timed out` en de gebruiker keek daar de volle timeout tegenaan (logs `xhs3j` en `kzq7c`, ruim veertig seconden tussen de laatste geslaagde en de mislukte schrijving) | `lib/screens/video_player/` afsluitpad, plus `reportPlaybackStopped` in `lib/services/pleya_server_client/parts/playback.dart` | de afsluitrapportage hoort losgekoppeld te worden van de UI-teardown: wegschrijven en bij falen naar de offline-wachtrij, niet de gebruiker laten wachten. Los oppakken, buiten PS-4 |
| De log-uploader dringt aan tegen een rate limit: `ice.pleya.app/logs` gaf tien keer achter elkaar `HTTP 429` binnen drie seconden (log `kzq7c`) | log-uploadpad in de client | backoff bij 429 in plaats van direct opnieuw; los oppakken |
